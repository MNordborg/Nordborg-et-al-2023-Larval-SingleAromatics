---
title: "2019.11 Larval Toluene metamorphosis"
...
```{r}
data.Tol.19Nov.meta.raw<- read_csv("Data/2019.11_AM_tolu_meta.csv") %>% 
  data.frame() %>% 
...
save(data.Tol.19Nov.meta.raw, file = "2019.11_Tol_data_meta_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.Tol.19Nov.meta.raw)
head(data.Tol.19Nov.meta.raw)
tail(data.Tol.19Nov.meta.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2019.11_Tol_data_meta_raw.RData")

#Remove treatment replicates from data set
data.Tol.19Nov.meta.controls <- data.Tol.19Nov.meta.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.Tol.19Nov.meta.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()

#Filer in preparation for t-tests
Tol.19Nov.meta.controls.PAR.FSW <- data.Tol.19Nov.meta.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x=="FSW")
Tol.19Nov.meta.controls.PAR.DMSO <- data.Tol.19Nov.meta.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x!="FSW")

Tol.19Nov.meta.controls.UV.FSW <- data.Tol.19Nov.meta.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x=="FSW")
Tol.19Nov.meta.controls.UV.DMSO <- data.Tol.19Nov.meta.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x!="FSW")

#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.Tol.19Nov.meta.controls, family = binomial())
...
data.Tol.19Nov.meta <- data.Tol.19Nov.meta.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.Tol.19Nov.meta.PAR.raw <- data.Tol.19Nov.meta %>% 
  dplyr::filter(factor=="PAR") 
data.Tol.19Nov.meta.PAR <- data.Tol.19Nov.meta.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.Tol.19Nov.meta.UV.raw <- data.Tol.19Nov.meta %>% 
  dplyr::filter(factor=="UVR") 
data.Tol.19Nov.meta.UV <- data.Tol.19Nov.meta.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.Tol.19Nov.meta.PAR)
View(data.Tol.19Nov.meta.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.Tol.19Nov.meta.PAR.raw, data.Tol.19Nov.meta.UV.raw, data.Tol.19Nov.meta.PAR,data.Tol.19Nov.meta.UV, file = "2019.11_Tol_data_meta_filtered.RData")

load(file = "2019.11_Tol_data_meta_filtered.RData")
```
...
unique(data.Tol.19Nov.meta.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.Tol.19Nov.meta.PAR$raw.x)
hist(data.Tol.19Nov.meta.PAR$suc/data.Tol.19Nov.meta.PAR$tot)
par(mfrow=c(1,1))
plot(data.Tol.19Nov.meta.PAR$log.x, data.Tol.19Nov.meta.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Tol.19Nov.meta.PAR.all <- bnec(data = data.Tol.19Nov.meta.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.Tol.19Nov.meta.PAR.all, file = "2019.11_Tol_meta_PAR_modfit_all.RData")
beep("mario")
```
...
```{r}
load("2019.11_Tol_meta_PAR_modfit_all.RData")

rhat(out.Tol.19Nov.meta.PAR.all, rhat_cutoff = 1.03)
```

Drop candidate models that failed the RHat test (if required)
```{r}
out.Tol.19Nov.meta.PAR.all2 <- amend(out.Tol.19Nov.meta.PAR.all, drop = c("nechorme4", "ecxll5" )) #add in bad candidate models in drop = c())
save(out.Tol.19Nov.meta.PAR.all, out.Tol.19Nov.meta.PAR.all2, file = "2019.11_Tol_meta_PAR_modfit_all.RData")
```
...
out.Tol.19Nov.meta.PAR.all2$mod_stats
summary(out.Tol.19Nov.meta.PAR.all2)
```
...
```{r}
plot(out.Tol.19Nov.meta.PAR.all2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```    
...
library(bayesnec)
library(beepr)
out.Tol.19Nov.meta.PAR.all.od <- bnec(data = data.Tol.19Nov.meta.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Tol.19Nov.meta.PAR.all.od, file = "2019.11_Tol_meta_PAR_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_Tol_meta_PAR_modfit_all_OD.RData")
rhat(out.Tol.19Nov.meta.PAR.all.od, rhat_cutoff = 1.03)
```

Drop candidate models that failed the RHat test (if required)
```{r}
out.Tol.19Nov.meta.PAR.all.od2 <- amend(out.Tol.19Nov.meta.PAR.all.od, drop = c("nechorme4")) #add in bad candidate models in drop = c())
save(out.Tol.19Nov.meta.PAR.all.od, out.Tol.19Nov.meta.PAR.all.od2, file = "2019.11_Tol_meta_PAR_modfit_all_OD.RData")
```
...
check_chains(out.Tol.19Nov.meta.PAR.all.od2, filename = "2019.11_Tol_meta_PAR_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: 
out.Tol.19Nov.meta.PAR.all.od2$mod_stats
summary(out.Tol.19Nov.meta.PAR.all.od2)
```
...
*Any evidence of non-convergence, poor chain mixing or autocorrelation?*
    ...
    - ecxhormebc5 =           exclude           (some evidence of por chain mixing)
    


```{r}
plot(out.Tol.19Nov.meta.PAR.all.od2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2019.11_Tol_meta_PAR_modfit_all_OD.RData")
pdf("2019.11_Tol_meta_PAR_modfitplots_all_OD.pdf")
plot(out.Tol.19Nov.meta.PAR.all.od2, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out.Tol.19Nov.meta.PAR.mod.od <- amend(out.Tol.19Nov.meta.PAR.all.od2, drop = c("neclinhorme", "ecxhormebc5"))
```
...
```{r}
plot(out.Tol.19Nov.meta.PAR.all.od2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out.Tol.19Nov.meta.PAR.mod.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
out.Tol.19Nov.meta.PAR <- out.Tol.19Nov.meta.PAR.mod.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Tol.19Nov.meta.PAR.NEC <- pull_out(out.Tol.19Nov.meta.PAR, model = c("nec"))
```


```{r}
plot(out.Tol.19Nov.meta.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Tol.19Nov.meta.PAR, out.Tol.19Nov.meta.PAR.NEC, file = "2019.11_Tol_meta_PAR_modfits_final.RData")
load("2019.11_Tol_meta_PAR_modfits_final.RData")
```
...
```{r}
load("2019.11_Tol_meta_PAR_modfits_final.RData")
NEC.Tol.19Nov.meta.PAR <- out.Tol.19Nov.meta.PAR.NEC$w_nec
NEC.Tol.19Nov.meta.PAR <- 10^NEC.Tol.19Nov.meta.PAR #if using log10(raw.x)
#NEC.Tol.19Nov.meta.PAR <- exp(NEC.Tol.19Nov.meta.PAR) #if using log(raw.x))
NEC.Tol.19Nov.meta.PAR
```

```{r}
load("2019.11_Tol_meta_PAR_modfits_final.RData")
EC10.Tol.19Nov.meta.PAR <- ecx(out.Tol.19Nov.meta.PAR, ecx_val = 10, type = "absolute")
EC10.Tol.19Nov.meta.PAR <- 10^EC10.Tol.19Nov.meta.PAR #if using log10(raw.x)
#EC10.Tol.19Nov.meta.PAR <- exp(EC10.Tol.19Nov.meta.PAR) #if using log(raw.x))
EC10.Tol.19Nov.meta.PAR
```

```{r}
load("2019.11_Tol_meta_PAR_modfits_final.RData")
EC50.Tol.19Nov.meta.PAR <- ecx(out.Tol.19Nov.meta.PAR, ecx_val = 50, type = "absolute")
EC50.Tol.19Nov.meta.PAR <- 10^EC50.Tol.19Nov.meta.PAR #if using log10(raw.x)
#EC50.Tol.19Nov.meta.PAR <- exp(EC50.Tol.19Nov.meta.PAR) #if using log(raw.x))
EC50.Tol.19Nov.meta.PAR
```
...
unique(data.Tol.19Nov.meta.UV$raw.x)
par(mfrow=c(2,1))
hist(data.Tol.19Nov.meta.UV$raw.x)
hist(data.Tol.19Nov.meta.UV$suc/data.Tol.19Nov.meta.UV$tot)
par(mfrow=c(1,1))
plot(data.Tol.19Nov.meta.UV$log.x, data.Tol.19Nov.meta.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Tol.19Nov.meta.UV.all <- bnec(data = data.Tol.19Nov.meta.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.Tol.19Nov.meta.UV.all, file = "2019.11_Tol_meta_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_Tol_meta_UV_modfit_all.RData")

rhat(out.Tol.19Nov.meta.UV.all, rhat_cutoff = 1.03)
```
...
out.Tol.19Nov.meta.UV.all$mod_stats
summary(out.Tol.19Nov.meta.UV.all)
```
...
```{r}
plot(out.Tol.19Nov.meta.UV.all, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.Tol.19Nov.meta.UV.all.od <- bnec(data = data.Tol.19Nov.meta.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Tol.19Nov.meta.UV.all.od, file = "2019.11_Tol_meta_UV_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_Tol_meta_UV_modfit_all_OD.RData")

rhat(out.Tol.19Nov.meta.UV.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.Tol.19Nov.meta.UV.all.od, filename = "2019.11_Tol_meta_UV_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: out.Tol.19Nov.meta.UV.all.od2
out.Tol.19Nov.meta.UV.all.od$mod_stats
summary(out.Tol.19Nov.meta.UV.all.od)
```

*Any evidence of non-convergence, poor chain mixing or autocorrelation?*
    - model =       include/unclear/exclude     (reason for potential exclusion)
    
    - nechorme4 =           include             (minor evidence of poor chain mixing)
...
```{r}
plot(out.Tol.19Nov.meta.UV.all.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2019.11_Tol_meta_UV_modfit_all_OD.RData")
pdf("2019.11_Tol_meta_UV_modfitplots_all_OD.pdf")
plot(out.Tol.19Nov.meta.UV.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
plot(out.Tol.19Nov.meta.UV.all, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10)
plot(out.Tol.19Nov.meta.UV.all.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10)
```
...
Rename model output for selected model (binomial or betabinomial)
```{r}
out.Tol.19Nov.meta.UV <- out.Tol.19Nov.meta.UV.all.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Tol.19Nov.meta.UV.NEC <- pull_out(out.Tol.19Nov.meta.UV, model = c("nec"))
```


```{r}
plot(out.Tol.19Nov.meta.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Tol.19Nov.meta.UV, out.Tol.19Nov.meta.UV.NEC, file = "2019.11_Tol_meta_UV_modfits_final.RData")
load("2019.11_Tol_meta_UV_modfits_final.RData")
```
...
```{r}
load("2019.11_Tol_meta_UV_modfits_final.RData")
NEC.Tol.19Nov.meta.UV <- out.Tol.19Nov.meta.UV.NEC$w_nec
NEC.Tol.19Nov.meta.UV <- 10^NEC.Tol.19Nov.meta.UV #if using log10(raw.x)
#NEC.Tol.19Nov.meta.UV <- exp(NEC.Tol.19Nov.meta.UV) #if using log(raw.x)
NEC.Tol.19Nov.meta.UV
```

```{r}
load("2019.11_Tol_meta_UV_modfits_final.RData")
EC10.Tol.19Nov.meta.UV <- ecx(out.Tol.19Nov.meta.UV, ecx_val = 10, type = "absolute")
EC10.Tol.19Nov.meta.UV <- 10^EC10.Tol.19Nov.meta.UV #if using log10(raw.x)
#EC10.Tol.19Nov.meta.UV <- exp(EC10.Tol.19Nov.meta.UV) #if using log(raw.x)
EC10.Tol.19Nov.meta.UV
```

```{r}
load("2019.11_Tol_meta_UV_modfits_final.RData")
EC50.Tol.19Nov.meta.UV <- ecx(out.Tol.19Nov.meta.UV, ecx_val = 50, type = "absolute")
EC50.Tol.19Nov.meta.UV <- 10^EC50.Tol.19Nov.meta.UV #if using log10(raw.x)
#EC50.Tol.19Nov.meta.UV <- exp(EC50.Tol.19Nov.meta.UV) #if using log(raw.x)
EC50.Tol.19Nov.meta.UV
```
...
pred.Tol.19Nov.meta <- compare_posterior(x=list("PAR" = out.Tol.19Nov.meta.PAR, "UVR" = out.Tol.19Nov.meta.UV), comparison = "fitted", precision = 50)

pred.Tol.19Nov.meta.PAR <- pred.Tol.19Nov.meta$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.Tol.19Nov.meta.UV <- pred.Tol.19Nov.meta$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.Tol.19Nov.meta.NEC <- compare_posterior(list("PAR" = out.Tol.19Nov.meta.PAR.NEC, "UVR" = out.Tol.19Nov.meta.UV.NEC), comparison = "fitted", precision = 50)

pred.Tol.19Nov.meta.PAR.NEC <- pred.Tol.19Nov.meta.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.Tol.19Nov.meta.UV.NEC <- pred.Tol.19Nov.meta.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.Tol.19Nov.meta, pred.Tol.19Nov.meta.PAR, pred.Tol.19Nov.meta.UV, pred.Tol.19Nov.meta.PAR.NEC, pred.Tol.19Nov.meta.UV.NEC, file = "2019.11_Tol_meta_predvals.RData") # pred.Tol.19Nov.meta.NEC, pred.Tol.19Nov.meta.PAR.NEC, pred.Tol.19Nov.meta.UV.NEC <- add these back in once UV-section is working

load("2019.11_Tol_meta_predvals.RData")
str(pred.Tol.19Nov.meta.PAR)
str(pred.Tol.19Nov.meta.UV)
```
...
diff.prob.Tol.19Nov.meta <- max(pred.Tol.19Nov.meta$prob_diff$prob)
diff.prob.Tol.19Nov.meta
```
...
pred.Tol.19Nov.meta$prob_diff$inv.x <- 10^pred.Tol.19Nov.meta$prob_diff$x

p.comp.prob= ggplot(pred.Tol.19Nov.meta$prob_diff) +
...
plot.Tol.19Nov.meta.diff.prob <- p.comp.prob
plot.Tol.19Nov.meta.diff.prob
```
...
p.all= p.all+ geom_point(data = data.Tol.19Nov.meta.PAR.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Tol.19Nov.meta.PAR.raw$tot*0.2, position=position_jitter(width = .025, height=0.03))
p.all= p.all+ geom_point(data = data.Tol.19Nov.meta.UV.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Tol.19Nov.meta.UV.raw$tot*0.22, position=position_jitter(width = .025, height=0.03))
...
p.NEC= p.NEC+ geom_point(data = data.Tol.19Nov.meta.PAR.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Tol.19Nov.meta.PAR.raw$tot*0.2, position=position_jitter(width = .025, height=0.03))
p.NEC= p.NEC+ geom_point(data = data.Tol.19Nov.meta.UV.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Tol.19Nov.meta.UV.raw$tot*0.22, position=position_jitter(width = .025, height=0.03))
...
comp.Tol.19Nov.meta.EC50 <- compare_posterior(x=list("PAR" = out.Tol.19Nov.meta.PAR, "UVR" = out.Tol.19Nov.meta.UV), comparison = "ecx", ecx_val = 50)
save(comp.Tol.19Nov.meta.EC50, file = "2019.11_Tol_meta_posterior_comp_EC50.RData")
comp.Tol.19Nov.meta.EC50$prob_diff
```
...
*Is EC50 +UVR lower than the EC50 -UVR?*
    - No, but there is a likelihood of 52.41% that it could happen
...
p.comp.EC50= p.comp.EC50+ annotate("text", label = "UVR < PAR = 52.41%", x = 0.55, y = 7.86, size = 2.4, colour = "grey20") #0.906 of the first annotation
...
p.comp.EC50.diff = ggplot(comp.Tol.19Nov.meta.EC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot.Tol.19Nov.meta.EC50.diff <- p.comp.EC50.diff
comp.plot.Tol.19Nov.meta.EC50.diff
```
...
```{r}
LC50.Tol.19Nov.meta.geomean <- average_endpoints(x=list("Tol_PAR" = out.Tol.19Nov.meta.PAR, "Tol_UV" = out.Tol.19Nov.meta.UV), endpoint = "ecx", ecx_val = 50, xform = inv.log10)
LC50.Tol.19Nov.meta.geomean
EC50.Tol.19Nov.meta.PAR
EC50.Tol.19Nov.meta.UV
```
...
comp.Tol.19Nov.meta.NEC <- compare_posterior(x=list("PAR" = out.Tol.19Nov.meta.PAR, "UVR" = out.Tol.19Nov.meta.UV), comparison = "nec")
save(comp.Tol.19Nov.meta.NEC, file = "2019.11_Tol_meta_posterior_comp_NEC.RData")
comp.Tol.19Nov.meta.NEC$prob_diff
```
Table shows the % likelihood that the point estimate calculated (NEC, EC/LC10 or EC/LC50) for model A (in top row) is smaller than the point estimate calculated for model B, C, etc (in left hand column). 

*Is the NEC +UVR lower than the NEC -UVR?*
    - No, it only has a likelihood of 18.38%.
...
p.comp.NEC= p.comp.NEC+ annotate("text", label = "UVR < PAR = 18.38%", x = 0.55, y = 7.86, size = 2.4, colour = "grey20") #0.906 of the first annotation
...
p.comp.NEC.diff = ggplot(comp.Tol.19Nov.meta.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.Tol.19Nov.meta.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.Tol.19Nov.meta.NEC.diff <- p.comp.NEC.diff
comp.plot.Tol.19Nov.meta.NEC.diff
```
...
```{r}
NEC.Tol.19Nov.meta.geomean <- average_endpoints(x=list("Tol_PAR" = out.Tol.19Nov.meta.PAR.NEC, "Tol_UV" = out.Tol.19Nov.meta.UV.NEC), endpoint = "nec", xform = inv.log10)
NEC.Tol.19Nov.meta.geomean
NEC.Tol.19Nov.meta.PAR
NEC.Tol.19Nov.meta.UV
```
...
```{r}
layout.Tol.19Nov.meta <- ggarrange(plot.Tol.19Nov.meta.all, comp.plot.Tol.19Nov.meta.EC50, plot.Tol.19Nov.meta.NEC, comp.plot.Tol.19Nov.meta.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.Tol.19Nov.meta
```
