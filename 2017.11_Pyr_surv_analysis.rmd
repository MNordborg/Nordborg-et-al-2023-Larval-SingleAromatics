...
out.Ant.17Dec.surv.UV.all2 <- amend(out.Ant.17Dec.surv.UV.all, drop = c("ecxll5")) #add in bad candidate models in drop = c())
...
out.Ant.17Dec.surv.UV.all.od2 <- amend(out.Ant.17Dec.surv.UV.all.od, drop = c("ecxll5")) #add in bad candidate models in drop = c())
...
```{r}
plot(out.Ant.17Dec.surv.UV.all.od2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
plot(out.Ant.17Dec.surv.UV.all.od2, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
plot(out.Ant.17Dec.surv.UV.all2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10)
plot(out.Ant.17Dec.surv.UV.all.od2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10)
```
...
Rename model output for selected model (binomial or betabinomial)
```{r}
out.Ant.17Dec.surv.UV <- out.Ant.17Dec.surv.UV.all.od2
```
...
*Is LC50 +UVR lower than the LC50 -UVR?*
    - Yes, with a likelihood of 85.85%.
...
p.comp.LC50 = p.comp.LC50 + scale_y_continuous(limits = c(-0.5, 12), name ="Posterior probability density") 
...
p.comp.LC50= p.comp.LC50+ annotate("text", label = expression(Probability~of~LC[50]~difference), x = 0.55, y = 9.6, size = 2.8, colour = "grey20") #0.8 of the max y-axis limit
p.comp.LC50= p.comp.LC50+ annotate("text", label = "UVR < PAR = 85.85%", x = 0.55, y = 8.7, size = 2.4, colour = "grey20") #0.906 of the first annotation
...
*Is the NEC +UVR lower than the NEC -UVR?*
    - Yes, with a likelihood of 76.54%.
...
p.comp.NEC= p.comp.NEC+ annotate("text", label = "UVR < PAR = 76.54%", x = 0.55, y = 1.45, size = 2.4, colour = "grey20") #0.906 of the first annotation
