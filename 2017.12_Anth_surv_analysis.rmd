---
title: "2017.11 Larval anthracene survival"
...
Replace: 
    .surv -> as appropriate for the endpoint
    _surv -> as appropriate for the endpoint
...
```{r}
data.Ant.17Dec.surv.raw<- read_csv("Data/2017.12_AM_anth_surv.csv") %>% 
  data.frame() %>% 
...
save(data.Ant.17Dec.surv.raw, file = "2017.12_Ant_data_surv_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.Ant.17Dec.surv.raw)
head(data.Ant.17Dec.surv.raw)
tail(data.Ant.17Dec.surv.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2017.12_Ant_data_surv_raw.RData")

#Remove treatment replicates from data set
data.Ant.17Dec.surv.controls <- data.Ant.17Dec.surv.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.Ant.17Dec.surv.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()

#Filer in preparation for t-tests
Ant.17Dec.surv.controls.PAR.FSW <- data.Ant.17Dec.surv.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x=="FSW")
Ant.17Dec.surv.controls.PAR.DMSO <- data.Ant.17Dec.surv.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x!="FSW")

Ant.17Dec.surv.controls.UV.FSW <- data.Ant.17Dec.surv.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x=="FSW")
Ant.17Dec.surv.controls.UV.DMSO <- data.Ant.17Dec.surv.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x!="FSW")

#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.Ant.17Dec.surv.controls, family = binomial())
...
Calculate the median larval survival using the posterior probability for each data set
```{r}
...
data.Ant.17Dec.surv <- data.Ant.17Dec.surv.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.Ant.17Dec.surv.PAR.raw <- data.Ant.17Dec.surv %>% 
  dplyr::filter(factor=="PAR") 
data.Ant.17Dec.surv.PAR <- data.Ant.17Dec.surv.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.Ant.17Dec.surv.UV.raw <- data.Ant.17Dec.surv %>% 
  dplyr::filter(factor=="UVR") 
data.Ant.17Dec.surv.UV <- data.Ant.17Dec.surv.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.Ant.17Dec.surv.PAR)
View(data.Ant.17Dec.surv.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.Ant.17Dec.surv.PAR.raw, data.Ant.17Dec.surv.UV.raw, data.Ant.17Dec.surv.PAR,data.Ant.17Dec.surv.UV, file = "2017.12_Ant_data_surv_filtered.RData")

load(file = "2017.12_Ant_data_surv_filtered.RData")
```
...
unique(data.Ant.17Dec.surv.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.Ant.17Dec.surv.PAR$raw.x)
hist(data.Ant.17Dec.surv.PAR$suc/data.Ant.17Dec.surv.PAR$tot)
par(mfrow=c(1,1))
plot(data.Ant.17Dec.surv.PAR$log.x, data.Ant.17Dec.surv.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Ant.17Dec.surv.PAR.all <- bnec(data = data.Ant.17Dec.surv.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.Ant.17Dec.surv.PAR.all, file = "2017.12_Ant_surv_PAR_modfit_all.RData")
beep("mario")
```
...
```{r}
load("2017.12_Ant_surv_PAR_modfit_all.RData")

rhat(out.Ant.17Dec.surv.PAR.all, rhat_cutoff = 1.03)
```
...
out.Ant.17Dec.surv.PAR.all$mod_stats
summary(out.Ant.17Dec.surv.PAR.all)
```

*Any evidence of overdispersion?*
    - All candidate models are somewhat overdispersed.


```{r}
plot(out.Ant.17Dec.surv.PAR.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```    
...
library(bayesnec)
library(beepr)
out.Ant.17Dec.surv.PAR.all.od <- bnec(data = data.Ant.17Dec.surv.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Ant.17Dec.surv.PAR.all.od, file = "2017.12_Ant_surv_PAR_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.12_Ant_surv_PAR_modfit_all_OD.RData")
rhat(out.Ant.17Dec.surv.PAR.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.Ant.17Dec.surv.PAR.all.od, filename = "2017.12_Ant_surv_PAR_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: 
out.Ant.17Dec.surv.PAR.all.od$mod_stats
summary(out.Ant.17Dec.surv.PAR.all.od)
```
...
```{r}
plot(out.Ant.17Dec.surv.PAR.all.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2017.12_Ant_surv_PAR_modfit_all_OD.RData")
pdf("2017.12_Ant_surv_PAR_modfitplots_all_OD.pdf")
plot(out.Ant.17Dec.surv.PAR.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out.Ant.17Dec.surv.PAR <- out.Ant.17Dec.surv.PAR.all.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Ant.17Dec.surv.PAR.NEC <- pull_out(out.Ant.17Dec.surv.PAR, model = c("nec"))
```


```{r}
plot(out.Ant.17Dec.surv.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Ant.17Dec.surv.PAR, out.Ant.17Dec.surv.PAR.NEC, file = "2017.12_Ant_surv_PAR_modfits_final.RData")
load("2017.12_Ant_surv_PAR_modfits_final.RData")
```
...
```{r}
load("2017.12_Ant_surv_PAR_modfits_final.RData")
NEC.Ant.17Dec.surv.PAR <- out.Ant.17Dec.surv.PAR.NEC$w_nec
NEC.Ant.17Dec.surv.PAR <- 10^NEC.Ant.17Dec.surv.PAR #if using log10(raw.x)
#NEC.Ant.17Dec.surv.PAR <- exp(NEC.Ant.17Dec.surv.PAR) #if using log(raw.x))
NEC.Ant.17Dec.surv.PAR
```

```{r}
load("2017.12_Ant_surv_PAR_modfits_final.RData")
LC10.Ant.17Dec.surv.PAR <- ecx(out.Ant.17Dec.surv.PAR, ecx_val = 10, type = "absolute")
LC10.Ant.17Dec.surv.PAR <- 10^LC10.Ant.17Dec.surv.PAR #if using log10(raw.x)
#LC10.Ant.17Dec.surv.PAR <- exp(LC10.Ant.17Dec.surv.PAR) #if using log(raw.x))
LC10.Ant.17Dec.surv.PAR
```

```{r}
load("2017.12_Ant_surv_PAR_modfits_final.RData")
LC50.Ant.17Dec.surv.PAR <- ecx(out.Ant.17Dec.surv.PAR, ecx_val = 50, type = "absolute")
LC50.Ant.17Dec.surv.PAR <- 10^LC50.Ant.17Dec.surv.PAR #if using log10(raw.x)
#LC50.Ant.17Dec.surv.PAR <- exp(LC50.Ant.17Dec.surv.PAR) #if using log(raw.x))
LC50.Ant.17Dec.surv.PAR
```
...
unique(data.Ant.17Dec.surv.UV$raw.x)
par(mfrow=c(2,1))
hist(data.Ant.17Dec.surv.UV$raw.x)
hist(data.Ant.17Dec.surv.UV$suc/data.Ant.17Dec.surv.UV$tot)
par(mfrow=c(1,1))
plot(data.Ant.17Dec.surv.UV$log.x, data.Ant.17Dec.surv.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Ant.17Dec.surv.UV.all <- bnec(data = data.Ant.17Dec.surv.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.Ant.17Dec.surv.UV.all, file = "2017.12_Ant_surv_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.12_Ant_surv_UV_modfit_all.RData")

rhat(out.Ant.17Dec.surv.UV.all, rhat_cutoff = 1.03)
```
...
out.Ant.17Dec.surv.UV.all2 <- amend(out.Ant.17Dec.surv.UV.all, drop = c("neclin", "ecxll5")) #add in bad candidate models in drop = c())
save(out.Ant.17Dec.surv.UV.all, out.Ant.17Dec.surv.UV.all2, file = "2017.12_Ant_surv_UV_modfit_all.RData")
```
...
out.Ant.17Dec.surv.UV.all2$mod_stats
summary(out.Ant.17Dec.surv.UV.all2)
```
...
```{r}
plot(out.Ant.17Dec.surv.UV.all2, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.Ant.17Dec.surv.UV.all.od <- bnec(data = data.Ant.17Dec.surv.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Ant.17Dec.surv.UV.all.od, file = "2017.12_Ant_surv_UV_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.12_Ant_surv_UV_modfit_all_OD.RData")

rhat(out.Ant.17Dec.surv.UV.all.od, rhat_cutoff = 1.03)
```
...
out.Ant.17Dec.surv.UV.all.od2 <- amend(out.Ant.17Dec.surv.UV.all.od, drop = c("neclin", "ecxll5")) #add in bad candidate models in drop = c())
save(out.Ant.17Dec.surv.UV.all.od, out.Ant.17Dec.surv.UV.all.od2, file = "2017.12_Ant_surv_UV_modfit_all_OD.RData")
```
...
check_chains(out.Ant.17Dec.surv.UV.all.od2, filename = "2017.12_Ant_surv_UV_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: out.Ant.17Dec.surv.UV.all.od2
out.Ant.17Dec.surv.UV.all.od2$mod_stats
summary(out.Ant.17Dec.surv.UV.all.od2)
```

*Any evidence of non-convergence, poor chain mixing or autocorrelation?*
    - model =       include/unclear/exclude     (reason for potential exclusion)
    
    - nechorme4 =         exclude               (evidence of non-convergence)
    ...
    - ecxhormebc5 =       include               (minor evidence of poor chain mixing)


```{r}
load("2017.12_Ant_surv_UV_modfit_all_OD.RData")

out.Ant.17Dec.surv.UV.mod.od <- amend(out.Ant.17Dec.surv.UV.all.od2, drop = c("nechorme4")) #add in bad candidate models in drop = c())
save(out.Ant.17Dec.surv.UV.all.od, out.Ant.17Dec.surv.UV.all.od2, out.Ant.17Dec.surv.UV.mod.od, file = "2017.12_Ant_surv_UV_modfit_all_OD.RData")
```


```{r}
plot(out.Ant.17Dec.surv.UV.mod.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2017.12_Ant_surv_UV_modfit_all_OD.RData")
pdf("2017.12_Ant_surv_UV_modfitplots_all_OD.pdf")
plot(out.Ant.17Dec.surv.UV.mod.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```


Compare the original model subset to the midified model subset
```{r}
plot(out.Ant.17Dec.surv.UV.all.od2, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10)
plot(out.Ant.17Dec.surv.UV.mod.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10)
```
...
```{r}
plot(out.Ant.17Dec.surv.UV.all2, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10)
plot(out.Ant.17Dec.surv.UV.mod.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10)
```
...
Rename model output for selected model (binomial or betabinomial)
```{r}
out.Ant.17Dec.surv.UV <- out.Ant.17Dec.surv.UV.mod.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Ant.17Dec.surv.UV.NEC <- pull_out(out.Ant.17Dec.surv.UV, model = c("nec"))
```


```{r}
plot(out.Ant.17Dec.surv.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Ant.17Dec.surv.UV, out.Ant.17Dec.surv.UV.NEC, file = "2017.12_Ant_surv_UV_modfits_final.RData")
load("2017.12_Ant_surv_UV_modfits_final.RData")
```
...
```{r}
load("2017.12_Ant_surv_UV_modfits_final.RData")
NEC.Ant.17Dec.surv.UV <- out.Ant.17Dec.surv.UV.NEC$w_nec
NEC.Ant.17Dec.surv.UV <- 10^NEC.Ant.17Dec.surv.UV #if using log10(raw.x)
#NEC.Ant.17Dec.surv.UV <- exp(NEC.Ant.17Dec.surv.UV) #if using log(raw.x)
NEC.Ant.17Dec.surv.UV
```

```{r}
load("2017.12_Ant_surv_UV_modfits_final.RData")
LC10.Ant.17Dec.surv.UV <- ecx(out.Ant.17Dec.surv.UV, ecx_val = 10, type = "absolute")
LC10.Ant.17Dec.surv.UV <- 10^LC10.Ant.17Dec.surv.UV #if using log10(raw.x)
#LC10.Ant.17Dec.surv.UV <- exp(LC10.Ant.17Dec.surv.UV) #if using log(raw.x)
LC10.Ant.17Dec.surv.UV
```

```{r}
load("2017.12_Ant_surv_UV_modfits_final.RData")
LC50.Ant.17Dec.surv.UV <- ecx(out.Ant.17Dec.surv.UV, ecx_val = 50, type = "absolute")
LC50.Ant.17Dec.surv.UV <- 10^LC50.Ant.17Dec.surv.UV #if using log10(raw.x)
#LC50.Ant.17Dec.surv.UV <- exp(LC50.Ant.17Dec.surv.UV) #if using log(raw.x)
LC50.Ant.17Dec.surv.UV
```
...
pred.Ant.17Dec.surv <- compare_posterior(x=list("PAR" = out.Ant.17Dec.surv.PAR, "UVR" = out.Ant.17Dec.surv.UV), comparison = "fitted", precision = 50)

pred.Ant.17Dec.surv.PAR <- pred.Ant.17Dec.surv$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.Ant.17Dec.surv.UV <- pred.Ant.17Dec.surv$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.Ant.17Dec.surv.NEC <- compare_posterior(list("PAR" = out.Ant.17Dec.surv.PAR.NEC, "UVR" = out.Ant.17Dec.surv.UV.NEC), comparison = "fitted", precision = 50)

pred.Ant.17Dec.surv.PAR.NEC <- pred.Ant.17Dec.surv.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.Ant.17Dec.surv.UV.NEC <- pred.Ant.17Dec.surv.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.Ant.17Dec.surv, pred.Ant.17Dec.surv.PAR, pred.Ant.17Dec.surv.UV, pred.Ant.17Dec.surv.PAR.NEC, pred.Ant.17Dec.surv.UV.NEC, file = "2017.12_Ant_surv_predvals.RData") # pred.Ant.17Dec.surv.NEC, pred.Ant.17Dec.surv.PAR.NEC, pred.Ant.17Dec.surv.UV.NEC <- add these back in once UV-section is working

load("2017.12_Ant_surv_predvals.RData")
str(pred.Ant.17Dec.surv.PAR)
str(pred.Ant.17Dec.surv.UV)
```
...
diff.prob.Ant.17Dec.surv <- max(pred.Ant.17Dec.surv$prob_diff$prob)
diff.prob.Ant.17Dec.surv
```
...
pred.Ant.17Dec.surv$prob_diff$inv.x <- 10^pred.Ant.17Dec.surv$prob_diff$x

p.comp.prob= ggplot(pred.Ant.17Dec.surv$prob_diff) +
...
plot.Ant.17Dec.surv.diff.prob <- p.comp.prob
plot.Ant.17Dec.surv.diff.prob
```
...
comp.Ant.17Dec.surv.LC50 <- compare_posterior(x=list("PAR" = out.Ant.17Dec.surv.PAR, "UVR" = out.Ant.17Dec.surv.UV), comparison = "ecx", ecx_val = 50)
save(comp.Ant.17Dec.surv.LC50, file = "2017.12_Ant_surv_posterior_comp_LC50.RData")
comp.Ant.17Dec.surv.LC50$prob_diff
```
...
*Is LC50 +UVR likely to be lower than the LC50 -UVR?*
    - No, with a likelihood of 26.63%.
...
p.comp.LC50.diff = ggplot(comp.Ant.17Dec.surv.LC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot.Ant.17Dec.surv.LC50.diff <- p.comp.LC50.diff
comp.plot.Ant.17Dec.surv.LC50.diff
```
...
comp.Ant.17Dec.surv.NEC <- compare_posterior(x=list("PAR" = out.Ant.17Dec.surv.PAR, "UVR" = out.Ant.17Dec.surv.UV), comparison = "nec")
save(comp.Ant.17Dec.surv.NEC, file = "2017.12_Ant_surv_posterior_comp_NEC.RData")
comp.Ant.17Dec.surv.NEC$prob_diff
```

    Table shows the % likelihood that the point estimate calculated (NEC, EC/LC10 or EC/LC50) for model A (in top row) is smaller than the point estimate calculated for model B, C, etc (in left hand column). 

*Is the NEC +UVR lower than the NEC -UVR?*
    - No, with a likelihood of 21.95%.
...
p.comp.NEC.diff = ggplot(comp.Ant.17Dec.surv.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.Ant.17Dec.surv.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.Ant.17Dec.surv.NEC.diff <- p.comp.NEC.diff
comp.plot.Ant.17Dec.surv.NEC.diff
```
...
```{r}
layout.Ant.17Dec.surv <- ggarrange(plot.Ant.17Dec.surv.all, comp.plot.Ant.17Dec.surv.LC50, plot.Ant.17Dec.surv.NEC, comp.plot.Ant.17Dec.surv.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.Ant.17Dec.surv
```
