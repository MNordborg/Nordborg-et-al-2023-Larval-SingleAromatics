---
title: "2017.11 Larval Pyrene metamorphosis"
...
Analysis performed using package bayesnec and it's dependencies in R Version 4.0.3. The latest version of bayesnec and all dependencies were installed prior to start of analysis (27 January 2021).
...
```{r}
data.Pyr.17Nov.meta.raw<- read_csv("Data/2017.11_AM_pyr_meta.csv") %>% 
  data.frame() %>% 
...
save(data.Pyr.17Nov.meta.raw, file = "2017.11_Pyr_data_meta_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.Pyr.17Nov.meta.raw)
head(data.Pyr.17Nov.meta.raw)
tail(data.Pyr.17Nov.meta.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2017.11_Pyr_data_meta_raw.RData")

#Remove treatment replicates from data set
data.Pyr.17Nov.meta.controls <- data.Pyr.17Nov.meta.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.Pyr.17Nov.meta.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()

#Filer in preparation for t-tests
Pyr.17Nov.meta.controls.PAR.FSW <- data.Pyr.17Nov.meta.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x=="FSW")
Pyr.17Nov.meta.controls.PAR.DMSO <- data.Pyr.17Nov.meta.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x!="FSW")

Pyr.17Nov.meta.controls.UV.FSW <- data.Pyr.17Nov.meta.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x=="FSW")
Pyr.17Nov.meta.controls.UV.DMSO <- data.Pyr.17Nov.meta.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x!="FSW")

#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.Pyr.17Nov.meta.controls, family = binomial())
...
data.Pyr.17Nov.meta <- data.Pyr.17Nov.meta.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.Pyr.17Nov.meta.PAR.raw <- data.Pyr.17Nov.meta %>% 
  dplyr::filter(factor=="PAR") 
data.Pyr.17Nov.meta.PAR <- data.Pyr.17Nov.meta.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.Pyr.17Nov.meta.UV.raw <- data.Pyr.17Nov.meta %>% 
  dplyr::filter(factor=="UVR") 
data.Pyr.17Nov.meta.UV <- data.Pyr.17Nov.meta.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.Pyr.17Nov.meta.PAR)
View(data.Pyr.17Nov.meta.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.Pyr.17Nov.meta.PAR.raw, data.Pyr.17Nov.meta.UV.raw, data.Pyr.17Nov.meta.PAR,data.Pyr.17Nov.meta.UV, file = "2017.11_Pyr_data_meta_filtered.RData")

load(file = "2017.11_Pyr_data_meta_filtered.RData")
```
...
unique(data.Pyr.17Nov.meta.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.Pyr.17Nov.meta.PAR$raw.x)
hist(data.Pyr.17Nov.meta.PAR$suc/data.Pyr.17Nov.meta.PAR$tot)
par(mfrow=c(1,1))
plot(data.Pyr.17Nov.meta.PAR$log.x, data.Pyr.17Nov.meta.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Pyr.17Nov.meta.PAR.all <- bnec(data = data.Pyr.17Nov.meta.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.Pyr.17Nov.meta.PAR.all, file = "2017.11_Pyr_meta_PAR_modfit_all.RData")
beep("mario")
```
...
```{r}
load("2017.11_Pyr_meta_PAR_modfit_all.RData")

rhat(out.Pyr.17Nov.meta.PAR.all, rhat_cutoff = 1.03)
```
...
out.Pyr.17Nov.meta.PAR.all$mod_stats
summary(out.Pyr.17Nov.meta.PAR.all)
```
...
library(bayesnec)
library(beepr)
out.Pyr.17Nov.meta.PAR.all.od <- bnec(data = data.Pyr.17Nov.meta.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Pyr.17Nov.meta.PAR.all.od, file = "2017.11_Pyr_meta_PAR_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.11_Pyr_meta_PAR_modfit_all_OD.RData")
rhat(out.Pyr.17Nov.meta.PAR.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.Pyr.17Nov.meta.PAR.all.od, filename = "2017.11_Pyr_meta_PAR_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: 
out.Pyr.17Nov.meta.PAR.all.od$mod_stats
summary(out.Pyr.17Nov.meta.PAR.all.od)
```


*Any evidence of non-convergence, poor chain mixing or autocorrelation?*
    - Some evidence of model fit being unstable (but no distinctly bad chain mixing)
    


```{r}
load("2017.11_Pyr_meta_PAR_modfit_all_OD.RData")
plot(out.Pyr.17Nov.meta.PAR.all.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2017.11_Pyr_meta_PAR_modfit_all_OD.RData")
pdf("2017.11_Pyr_meta_PAR_modfitplots_all_OD.pdf")
plot(out.Pyr.17Nov.meta.PAR.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
plot(out.Pyr.17Nov.meta.PAR.all, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
plot(out.Pyr.17Nov.meta.PAR.all.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
```{r}
out.Pyr.17Nov.meta.PAR <- out.Pyr.17Nov.meta.PAR.all.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Pyr.17Nov.meta.PAR.NEC <- pull_out(out.Pyr.17Nov.meta.PAR, model = c("nec"))
```


```{r}
plot(out.Pyr.17Nov.meta.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Pyr.17Nov.meta.PAR, out.Pyr.17Nov.meta.PAR.NEC, file = "2017.11_Pyr_meta_PAR_modfits_final.RData")
load("2017.11_Pyr_meta_PAR_modfits_final.RData")
```
...
```{r}
load("2017.11_Pyr_meta_PAR_modfits_final.RData")
NEC.Anth.16Nov.meta.PAR <- out.Pyr.17Nov.meta.PAR.NEC$w_nec
NEC.Anth.16Nov.meta.PAR <- 10^NEC.Anth.16Nov.meta.PAR #if using log10(raw.x)
#NEC.Anth.16Nov.meta.PAR <- exp(NEC.Anth.16Nov.meta.PAR) #if using log(raw.x))
NEC.Anth.16Nov.meta.PAR
```

```{r}
load("2017.11_Pyr_meta_PAR_modfits_final.RData")
EC10.Anth.16Nov.meta.PAR <- ecx(out.Pyr.17Nov.meta.PAR, ecx_val = 10, type = "absolute")
EC10.Anth.16Nov.meta.PAR <- 10^EC10.Anth.16Nov.meta.PAR #if using log10(raw.x)
#EC10.Anth.16Nov.meta.PAR <- exp(EC10.Anth.16Nov.meta.PAR) #if using log(raw.x))
EC10.Anth.16Nov.meta.PAR
```

```{r}
load("2017.11_Pyr_meta_PAR_modfits_final.RData")
EC50.Anth.16Nov.meta.PAR <- ecx(out.Pyr.17Nov.meta.PAR, ecx_val = 50, type = "absolute")
EC50.Anth.16Nov.meta.PAR <- 10^EC50.Anth.16Nov.meta.PAR #if using log10(raw.x)
#EC50.Anth.16Nov.meta.PAR <- exp(EC50.Anth.16Nov.meta.PAR) #if using log(raw.x))
EC50.Anth.16Nov.meta.PAR
```
...
unique(data.Pyr.17Nov.meta.UV$raw.x)
par(mfrow=c(2,1))
hist(data.Pyr.17Nov.meta.UV$raw.x)
hist(data.Pyr.17Nov.meta.UV$suc/data.Pyr.17Nov.meta.UV$tot)
par(mfrow=c(1,1))
plot(data.Pyr.17Nov.meta.UV$log.x, data.Pyr.17Nov.meta.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Pyr.17Nov.meta.UV.all <- bnec(data = data.Pyr.17Nov.meta.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.Pyr.17Nov.meta.UV.all, file = "2017.11_Pyr_meta_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.11_Pyr_meta_UV_modfit_all.RData")

rhat(out.Pyr.17Nov.meta.UV.all, rhat_cutoff = 1.03)
```
...
out.Pyr.17Nov.meta.UV.all$mod_stats
summary(out.Pyr.17Nov.meta.UV.all)
```
...
```{r}
plot(out.Pyr.17Nov.meta.UV.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.Pyr.17Nov.meta.UV.all.od <- bnec(data = data.Pyr.17Nov.meta.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Pyr.17Nov.meta.UV.all.od, file = "2017.11_Pyr_meta_UV_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2017.11_Pyr_meta_UV_modfit_all_OD.RData")

rhat(out.Pyr.17Nov.meta.UV.all.od, rhat_cutoff = 1.03)
```
...
out.Pyr.17Nov.meta.UV.all.od2 <- amend(out.Pyr.17Nov.meta.UV.all.od, drop = c("ecxll5" )) #add in bad candidate models in drop = c())
save(out.Pyr.17Nov.meta.UV.all.od, out.Pyr.17Nov.meta.UV.all.od2, file = "2017.11_Pyr_meta_UV_modfit_all_OD.RData")
```
...
check_chains(out.Pyr.17Nov.meta.UV.all.od2, filename = "2017.11_Pyr_meta_UV_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: out.Pyr.17Nov.meta.UV.all.od2
out.Pyr.17Nov.meta.UV.all.od2$mod_stats
```

*Any evidence of non-convergence, poor chain mixing or autocorrelation?*
    - model =         include/unclear/exclude     (reason for potential exclusion)
    
        - ecxwb1 =          include           (very minor evidence of poor chain mixing)
        - ecxll4 =          include           (very minor evidence of poor chain mixing)
        - ecxhormebc5 =     exclude           (evidence of poor chain mixing)
        


```{r}
load("2017.11_Pyr_meta_UV_modfit_all_OD.RData")
plot(out.Pyr.17Nov.meta.UV.all.od2, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2017.11_Pyr_meta_UV_modfit_all_OD.RData")
pdf("2017.11_Pyr_meta_UV_modfitplots_all_OD.pdf")
plot(out.Pyr.17Nov.meta.UV.all.od2, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
dev.off()
```

Any models that are inappropriate for the data based on the model median and CIs?
    - ecxlin


Drop inappropriate models and models with non-convergence or very poor chain mixing (if required)
```{r}
out.Pyr.17Nov.meta.UV.mod.od <- amend(out.Pyr.17Nov.meta.UV.all.od2, drop = c("ecxhormebc5", "ecxlin"))
```


Compare MANEC containing all OD candidate models to the amended MANEC:
```{r}
plot(out.Pyr.17Nov.meta.UV.all.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
plot(out.Pyr.17Nov.meta.UV.mod.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```
...
```{r}
plot(out.Pyr.17Nov.meta.UV.all, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10)
plot(out.Pyr.17Nov.meta.UV.mod.od, add_ec10 = FALSE, add_nec = FALSE, lxform = inv.log10)
```
...
Rename model output for selected model (binomial or betabinomial)
```{r}
out.Pyr.17Nov.meta.UV <- out.Pyr.17Nov.meta.UV.mod.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Pyr.17Nov.meta.UV.NEC <- pull_out(out.Pyr.17Nov.meta.UV, model = c("nec"))
```


```{r}
plot(out.Pyr.17Nov.meta.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modeled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Pyr.17Nov.meta.UV, out.Pyr.17Nov.meta.UV.NEC, file = "2017.11_Pyr_meta_UV_modfits_final.RData")
load("2017.11_Pyr_meta_UV_modfits_final.RData")
```
...
```{r}
load("2017.11_Pyr_meta_UV_modfits_final.RData")
NEC.Anth.16Nov.meta.UV <- out.Pyr.17Nov.meta.UV.NEC$w_nec
NEC.Anth.16Nov.meta.UV <- 10^NEC.Anth.16Nov.meta.UV #if using log10(raw.x)
#NEC.Anth.16Nov.meta.UV <- exp(NEC.Anth.16Nov.meta.UV) #if using log(raw.x)
NEC.Anth.16Nov.meta.UV
```

```{r}
load("2017.11_Pyr_meta_UV_modfits_final.RData")
EC10.Anth.16Nov.meta.UV <- ecx(out.Pyr.17Nov.meta.UV, ecx_val = 10, type = "absolute")
EC10.Anth.16Nov.meta.UV <- 10^EC10.Anth.16Nov.meta.UV #if using log10(raw.x)
#EC10.Anth.16Nov.meta.UV <- exp(EC10.Anth.16Nov.meta.UV) #if using log(raw.x)
EC10.Anth.16Nov.meta.UV
```

```{r}
load("2017.11_Pyr_meta_UV_modfits_final.RData")
EC50.Anth.16Nov.meta.UV <- ecx(out.Pyr.17Nov.meta.UV, ecx_val = 50, type = "absolute")
EC50.Anth.16Nov.meta.UV <- 10^EC50.Anth.16Nov.meta.UV #if using log10(raw.x)
#EC50.Anth.16Nov.meta.UV <- exp(EC50.Anth.16Nov.meta.UV) #if using log(raw.x)
EC50.Anth.16Nov.meta.UV
```
...
pred.Pyr.17Nov.meta <- compare_posterior(x=list("PAR" = out.Pyr.17Nov.meta.PAR, "UVR" = out.Pyr.17Nov.meta.UV), comparison = "fitted", precision = 50)

pred.Pyr.17Nov.meta.PAR <- pred.Pyr.17Nov.meta$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.Pyr.17Nov.meta.UV <- pred.Pyr.17Nov.meta$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.Pyr.17Nov.meta.NEC <- compare_posterior(list("PAR" = out.Pyr.17Nov.meta.PAR.NEC, "UVR" = out.Pyr.17Nov.meta.UV.NEC), comparison = "fitted", precision = 50)

pred.Pyr.17Nov.meta.PAR.NEC <- pred.Pyr.17Nov.meta.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.Pyr.17Nov.meta.UV.NEC <- pred.Pyr.17Nov.meta.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.Pyr.17Nov.meta, pred.Pyr.17Nov.meta.PAR, pred.Pyr.17Nov.meta.UV, pred.Pyr.17Nov.meta.PAR.NEC, pred.Pyr.17Nov.meta.UV.NEC, file = "2017.11_Pyr_meta_predvals.RData") # pred.Pyr.17Nov.meta.NEC, pred.Pyr.17Nov.meta.PAR.NEC, pred.Pyr.17Nov.meta.UV.NEC <- add these back in once UV-section is working

load("2017.11_Pyr_meta_predvals.RData")
str(pred.Pyr.17Nov.meta.PAR)
str(pred.Pyr.17Nov.meta.UV)
```
...
p.all= p.all+ geom_point(data = data.Pyr.17Nov.meta.PAR.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Pyr.17Nov.meta.PAR.raw$tot*0.2, position=position_jitter(width = .025, height=0.03))
p.all= p.all+ geom_point(data = data.Pyr.17Nov.meta.UV.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Pyr.17Nov.meta.UV.raw$tot*0.22, position=position_jitter(width = .025, height=0.03))
...
p.NEC= p.NEC+ geom_point(data = data.Pyr.17Nov.meta.PAR.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Pyr.17Nov.meta.PAR.raw$tot*0.2, position=position_jitter(width = .025, height=0.03))
p.NEC= p.NEC+ geom_point(data = data.Pyr.17Nov.meta.UV.raw, aes(x = raw.x, y = prop, colour=factor(factor), shape = factor(factor), alpha = 0.3), size = data.Pyr.17Nov.meta.UV.raw$tot*0.22, position=position_jitter(width = .025, height=0.03))
...
comp.Pyr.17Nov.meta.EC50 <- compare_posterior(x=list("PAR" = out.Pyr.17Nov.meta.PAR, "UVR" = out.Pyr.17Nov.meta.UV), comparison = "ecx", ecx_val = 50)
save(comp.Pyr.17Nov.meta.EC50, file = "2017.11_Pyr_meta_posterior_comp_EC50.RData")
comp.Pyr.17Nov.meta.EC50$prob_diff
```
...
*Is EC50 +UVR lower than the EC50 -UVR?*
    - Yes, with a likelihood of 83.63%
...
p.comp.EC50= p.comp.EC50+ annotate("text", label = "UVR < PAR = 83.63%", x = 0.55, y = 6.86, size = 2.4, colour = "grey20") #0.906 of the first annotation
...
p.comp.EC50.diff = ggplot(comp.Pyr.17Nov.meta.EC50$diff_data, aes(x=diff)) +
...
#Rename & show plot
comp.plot.Pyr.17Nov.meta.EC50.diff <- p.comp.EC50.diff
comp.plot.Pyr.17Nov.meta.EC50.diff
```
...
comp.Pyr.17Nov.meta.NEC <- compare_posterior(x=list("PAR" = out.Pyr.17Nov.meta.PAR, "UVR" = out.Pyr.17Nov.meta.UV), comparison = "nec")
save(comp.Pyr.17Nov.meta.NEC, file = "2017.11_Pyr_meta_posterior_comp_NEC.RData")
comp.Pyr.17Nov.meta.NEC$prob_diff
```
Table shows the % likelihood that the point estimate calculated (NEC, EC/LC10 or EC/LC50) for model A (in top row) is smaller than the point estimate calculated for model B, C, etc (in left hand column). 

*Is the NEC +UVR lower than the NEC -UVR?*
    - Yes, with a likelihood of 70.25%.
...
p.comp.NEC= p.comp.NEC+ annotate("text", label = "UVR < PAR = 70.25%", x = 0.55, y = 6.86, size = 2.4, colour = "grey20") #0.906 of the first annotation
...
p.comp.NEC.diff = ggplot(comp.Pyr.17Nov.meta.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.Pyr.17Nov.meta.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.Pyr.17Nov.meta.NEC.diff <- p.comp.NEC.diff
comp.plot.Pyr.17Nov.meta.NEC.diff
```
...
diff.prob.Pyr.17Nov.meta <- max(pred.Pyr.17Nov.meta$prob_diff$prob)
diff.prob.Pyr.17Nov.meta
```
...
```{r}
layout.Pyr.17Nov.meta <- ggarrange(plot.Pyr.17Nov.meta.all, comp.plot.Pyr.17Nov.meta.EC50, plot.Pyr.17Nov.meta.NEC, comp.plot.Pyr.17Nov.meta.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.Pyr.17Nov.meta
```
