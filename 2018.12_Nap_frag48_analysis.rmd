---
title: "2017.11 Larval Naphthalene 48 h fragmentation"
...
```{r}
data.Nap.18Dec.frag48.raw<- read_csv("Data/2018.12_AM_naph_frags48h.csv") %>% 
  data.frame() %>% 
...
save(data.Nap.18Dec.frag48.raw, file = "2018.12_Nap_data_frag48_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.Nap.18Dec.frag48.raw)
head(data.Nap.18Dec.frag48.raw)
tail(data.Nap.18Dec.frag48.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2018.12_Nap_data_frag48_raw.RData")

#Remove treatment replicates from data set
data.Nap.18Dec.frag48.controls <- data.Nap.18Dec.frag48.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.Nap.18Dec.frag48.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()

#Filer in preparation for t-tests
Nap.18Dec.frag48.controls.PAR.FSW <- data.Nap.18Dec.frag48.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x=="FSW")
Nap.18Dec.frag48.controls.PAR.DMSO <- data.Nap.18Dec.frag48.controls %>% 
  dplyr::filter(factor=="PAR" & nom.x!="FSW")

Nap.18Dec.frag48.controls.UV.FSW <- data.Nap.18Dec.frag48.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x=="FSW")
Nap.18Dec.frag48.controls.UV.DMSO <- data.Nap.18Dec.frag48.controls %>% 
  dplyr::filter(factor=="UVR" & nom.x!="FSW")

#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.Nap.18Dec.frag48.controls, family = binomial())
...
data.Nap.18Dec.frag48 <- data.Nap.18Dec.frag48.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.Nap.18Dec.frag48.PAR.raw <- data.Nap.18Dec.frag48 %>% 
  dplyr::filter(factor=="PAR") 
data.Nap.18Dec.frag48.PAR <- data.Nap.18Dec.frag48.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.Nap.18Dec.frag48.UV.raw <- data.Nap.18Dec.frag48 %>% 
  dplyr::filter(factor=="UVR") 
data.Nap.18Dec.frag48.UV <- data.Nap.18Dec.frag48.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.Nap.18Dec.frag48.PAR)
View(data.Nap.18Dec.frag48.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.Nap.18Dec.frag48.PAR.raw, data.Nap.18Dec.frag48.UV.raw, data.Nap.18Dec.frag48.PAR,data.Nap.18Dec.frag48.UV, file = "2018.12_Nap_data_frag48_filtered.RData")

load(file = "2018.12_Nap_data_frag48_filtered.RData")
```
...
unique(data.Nap.18Dec.frag48.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.Nap.18Dec.frag48.PAR$raw.x)
hist(data.Nap.18Dec.frag48.PAR$suc/data.Nap.18Dec.frag48.PAR$tot)
par(mfrow=c(1,1))
plot(data.Nap.18Dec.frag48.PAR$log.x, data.Nap.18Dec.frag48.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Nap.18Dec.frag48.PAR.all <- bnec(data = data.Nap.18Dec.frag48.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.Nap.18Dec.frag48.PAR.all, file = "2018.12_Nap_frag48_PAR_modfit_all.RData")
beep("mario")
```
...
```{r}
load("2018.12_Nap_frag48_PAR_modfit_all.RData")

rhat(out.Nap.18Dec.frag48.PAR.all, rhat_cutoff = 1.03)
```
...
out.Nap.18Dec.frag48.PAR.all$mod_stats
summary(out.Nap.18Dec.frag48.PAR.all)
```
...
library(bayesnec)
library(beepr)
out.Nap.18Dec.frag48.PAR.all.od <- bnec(data = data.Nap.18Dec.frag48.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Nap.18Dec.frag48.PAR.all.od, file = "2018.12_Nap_frag48_PAR_modfit_all_OD.RData")
beep("mario")
```

No candidate model could be successfully fitted using the betabinomial distribution.


###### Model selection

As no candidate models could be fitted using the betabinomial distribution the MANEC fitted using the binomial distribution was used instead.


####### Continue model evaluations for MANEC fitted using the binomial distribution
Check chain mixing
```{r}
load("2018.12_Nap_frag48_PAR_modfit_all.RData")

check_chains(out.Nap.18Dec.frag48.PAR.all, filename = "2018.12_Nap_frag48_PAR_chains_modfit_all") #change to out.Nap.18Dec.frag48.PAR.all2 if any models were dropped based on rhat
```
...
```{r}
plot(out.Nap.18Dec.frag48.PAR.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```    
...
pdf("2018.12_Nap_frag48_PAR_modfitplots_all.pdf")
plot(out.Nap.18Dec.frag48.PAR.all, all_models = TRUE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```


####### Rename and save selected model output


Rename model output for selected model (binomial or betabinomial)
```{r}
out.Nap.18Dec.frag48.PAR <- out.Nap.18Dec.frag48.PAR.all
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Nap.18Dec.frag48.PAR.NEC <- pull_out(out.Nap.18Dec.frag48.PAR, model = c("nec"))
```


```{r}
plot(out.Nap.18Dec.frag48.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Nap.18Dec.frag48.PAR, out.Nap.18Dec.frag48.PAR.NEC, file = "2018.12_Nap_frag48_PAR_modfits_final.RData")
load("2018.12_Nap_frag48_PAR_modfits_final.RData")
```
...
```{r}
load("2018.12_Nap_frag48_PAR_modfits_final.RData")
NEC.Nap.18Dec.frag48.PAR <- out.Nap.18Dec.frag48.PAR.NEC$w_nec
NEC.Nap.18Dec.frag48.PAR <- 10^NEC.Nap.18Dec.frag48.PAR #if using log10(raw.x)
#NEC.Nap.18Dec.frag48.PAR <- exp(NEC.Nap.18Dec.frag48.PAR) #if using log(raw.x))
NEC.Nap.18Dec.frag48.PAR
```

```{r}
load("2018.12_Nap_frag48_PAR_modfits_final.RData")
EC10.Nap.18Dec.frag48.PAR <- ecx(out.Nap.18Dec.frag48.PAR, ecx_val = 10, type = "absolute")
EC10.Nap.18Dec.frag48.PAR <- 10^EC10.Nap.18Dec.frag48.PAR #if using log10(raw.x)
#EC10.Nap.18Dec.frag48.PAR <- exp(EC10.Nap.18Dec.frag48.PAR) #if using log(raw.x))
EC10.Nap.18Dec.frag48.PAR
```

```{r}
load("2018.12_Nap_frag48_PAR_modfits_final.RData")
EC50.Nap.18Dec.frag48.PAR <- ecx(out.Nap.18Dec.frag48.PAR, ecx_val = 50, type = "absolute")
EC50.Nap.18Dec.frag48.PAR <- 10^EC50.Nap.18Dec.frag48.PAR #if using log10(raw.x)
#EC50.Nap.18Dec.frag48.PAR <- exp(EC50.Nap.18Dec.frag48.PAR) #if using log(raw.x))
EC50.Nap.18Dec.frag48.PAR
```
...
unique(data.Nap.18Dec.frag48.UV$raw.x)
par(mfrow=c(2,1))
hist(data.Nap.18Dec.frag48.UV$raw.x)
hist(data.Nap.18Dec.frag48.UV$suc/data.Nap.18Dec.frag48.UV$tot)
par(mfrow=c(1,1))
plot(data.Nap.18Dec.frag48.UV$log.x, data.Nap.18Dec.frag48.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.Nap.18Dec.frag48.UV.all <- bnec(data = data.Nap.18Dec.frag48.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.Nap.18Dec.frag48.UV.all, file = "2018.12_Nap_frag48_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2018.12_Nap_frag48_UV_modfit_all.RData")

rhat(out.Nap.18Dec.frag48.UV.all, rhat_cutoff = 1.03)
```
...
out.Nap.18Dec.frag48.UV.all$mod_stats
summary(out.Nap.18Dec.frag48.UV.all)
```

*Any evidence of overdispersion?*
    - All candidate models showed evidence of overdispersion.


```{r}
plot(out.Nap.18Dec.frag48.UV.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.Nap.18Dec.frag48.UV.all.od <- bnec(data = data.Nap.18Dec.frag48.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.Nap.18Dec.frag48.UV.all.od, file = "2018.12_Nap_frag48_UV_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2018.12_Nap_frag48_UV_modfit_all_OD.RData")

rhat(out.Nap.18Dec.frag48.UV.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.Nap.18Dec.frag48.UV.all.od, filename = "2018.12_Nap_frag48_UV_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: out.Nap.18Dec.frag48.UV.all.od2
out.Nap.18Dec.frag48.UV.all.od$mod_stats
summary(out.Nap.18Dec.frag48.UV.all.od)
```


```{r}
plot(out.Nap.18Dec.frag48.UV.all.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2018.12_Nap_frag48_UV_modfit_all_OD.RData")
pdf("2018.12_Nap_frag48_UV_modfitplots_all_OD.pdf")
plot(out.Nap.18Dec.frag48.UV.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out.Nap.18Dec.frag48.UV.mod.od <- amend(out.Nap.18Dec.frag48.UV.all.od, drop = c("nechorme4", "neclinhorme", "ecxhormebc5", "ecxlin"))
```
...
```{r}
plot(out.Nap.18Dec.frag48.UV.all.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out.Nap.18Dec.frag48.UV.mod.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
Compare MANEC fit for mdoels fitted on binomial and betabinomial distribution:
```{r}
plot(out.Nap.18Dec.frag48.UV.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
plot(out.Nap.18Dec.frag48.UV.mod.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```

MANEC fitted using the betabinomial distribution gave a more appropriate credible interval than MANEC fitted using the binomial distribution.


Rename model output for selected model (binomial or betabinomial)
```{r}
out.Nap.18Dec.frag48.UV <- out.Nap.18Dec.frag48.UV.mod.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.Nap.18Dec.frag48.UV.NEC <- pull_out(out.Nap.18Dec.frag48.UV, model = c("nec"))
```


```{r}
plot(out.Nap.18Dec.frag48.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.Nap.18Dec.frag48.UV, out.Nap.18Dec.frag48.UV.NEC, file = "2018.12_Nap_frag48_UV_modfits_final.RData")
load("2018.12_Nap_frag48_UV_modfits_final.RData")
```
...
```{r}
load("2018.12_Nap_frag48_UV_modfits_final.RData")
NEC.Nap.18Dec.frag48.UV <- out.Nap.18Dec.frag48.UV.NEC$w_nec
NEC.Nap.18Dec.frag48.UV <- 10^NEC.Nap.18Dec.frag48.UV #if using log10(raw.x)
#NEC.Nap.18Dec.frag48.UV <- exp(NEC.Nap.18Dec.frag48.UV) #if using log(raw.x)
NEC.Nap.18Dec.frag48.UV
```

```{r}
load("2018.12_Nap_frag48_UV_modfits_final.RData")
EC10.Nap.18Dec.frag48.UV <- ecx(out.Nap.18Dec.frag48.UV, ecx_val = 10, type = "absolute")
EC10.Nap.18Dec.frag48.UV <- 10^EC10.Nap.18Dec.frag48.UV #if using log10(raw.x)
#EC10.Nap.18Dec.frag48.UV <- exp(EC10.Nap.18Dec.frag48.UV) #if using log(raw.x)
EC10.Nap.18Dec.frag48.UV
```

```{r}
load("2018.12_Nap_frag48_UV_modfits_final.RData")
EC50.Nap.18Dec.frag48.UV <- ecx(out.Nap.18Dec.frag48.UV, ecx_val = 50, type = "absolute")
EC50.Nap.18Dec.frag48.UV <- 10^EC50.Nap.18Dec.frag48.UV #if using log10(raw.x)
#EC50.Nap.18Dec.frag48.UV <- exp(EC50.Nap.18Dec.frag48.UV) #if using log(raw.x)
EC50.Nap.18Dec.frag48.UV
```
...
pred.Nap.18Dec.frag48 <- compare_posterior(x=list("PAR" = out.Nap.18Dec.frag48.PAR, "UVR" = out.Nap.18Dec.frag48.UV), comparison = "fitted", precision = 50)

pred.Nap.18Dec.frag48.PAR <- pred.Nap.18Dec.frag48$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.Nap.18Dec.frag48.UV <- pred.Nap.18Dec.frag48$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.Nap.18Dec.frag48.NEC <- compare_posterior(list("PAR" = out.Nap.18Dec.frag48.PAR.NEC, "UVR" = out.Nap.18Dec.frag48.UV.NEC), comparison = "fitted", precision = 50)

pred.Nap.18Dec.frag48.PAR.NEC <- pred.Nap.18Dec.frag48.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.Nap.18Dec.frag48.UV.NEC <- pred.Nap.18Dec.frag48.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.Nap.18Dec.frag48, pred.Nap.18Dec.frag48.PAR, pred.Nap.18Dec.frag48.UV, pred.Nap.18Dec.frag48.PAR.NEC, pred.Nap.18Dec.frag48.UV.NEC, file = "2018.12_Nap_frag48_predvals.RData") # pred.Nap.18Dec.frag48.NEC, pred.Nap.18Dec.frag48.PAR.NEC, pred.Nap.18Dec.frag48.UV.NEC <- add these back in once UV-section is working

load("2018.12_Nap_frag48_predvals.RData")
str(pred.Nap.18Dec.frag48.PAR)
str(pred.Nap.18Dec.frag48.UV)
```


#### Plot MANEC model fits alongside raw data

#####Invert predicted response

Invert model median and credible intervals for full subset MANECs
```{r}
pred.Nap.18Dec.frag48.PAR.inv <- as.data.frame(pred.Nap.18Dec.frag48.PAR)
head(pred.Nap.18Dec.frag48.PAR.inv)
pred.Nap.18Dec.frag48.PAR.inv <- mutate(pred.Nap.18Dec.frag48.PAR.inv, Estimate = 1-(pred.Nap.18Dec.frag48.PAR.inv$Estimate))
pred.Nap.18Dec.frag48.PAR.inv <- mutate(pred.Nap.18Dec.frag48.PAR.inv, Q97.5 = 1-(pred.Nap.18Dec.frag48.PAR.inv$Q97.5))
pred.Nap.18Dec.frag48.PAR.inv <- mutate(pred.Nap.18Dec.frag48.PAR.inv, Q2.5 = 1-(pred.Nap.18Dec.frag48.PAR.inv$Q2.5))
head(pred.Nap.18Dec.frag48.PAR.inv)
```

```{r}
pred.Nap.18Dec.frag48.UV.inv <- as.data.frame(pred.Nap.18Dec.frag48.UV)
head(pred.Nap.18Dec.frag48.UV.inv)
pred.Nap.18Dec.frag48.UV.inv <- mutate(pred.Nap.18Dec.frag48.UV.inv, Estimate = 1-(pred.Nap.18Dec.frag48.UV.inv$Estimate))
pred.Nap.18Dec.frag48.UV.inv <- mutate(pred.Nap.18Dec.frag48.UV.inv, Q97.5 = 1-(pred.Nap.18Dec.frag48.UV.inv$Q97.5))
pred.Nap.18Dec.frag48.UV.inv <- mutate(pred.Nap.18Dec.frag48.UV.inv, Q2.5 = 1-(pred.Nap.18Dec.frag48.UV.inv$Q2.5))
head(pred.Nap.18Dec.frag48.UV.inv)
```

Invert model median and credible intervals for NEC subset MANECs
```{r}
pred.Nap.18Dec.frag48.PAR.NEC.inv <- as.data.frame(pred.Nap.18Dec.frag48.PAR)
head(pred.Nap.18Dec.frag48.PAR.NEC.inv)
pred.Nap.18Dec.frag48.PAR.NEC.inv <- mutate(pred.Nap.18Dec.frag48.PAR.NEC.inv, Estimate = 1-(pred.Nap.18Dec.frag48.PAR.NEC.inv$Estimate))
pred.Nap.18Dec.frag48.PAR.NEC.inv <- mutate(pred.Nap.18Dec.frag48.PAR.NEC.inv, Q97.5 = 1-(pred.Nap.18Dec.frag48.PAR.NEC.inv$Q97.5))
pred.Nap.18Dec.frag48.PAR.NEC.inv <- mutate(pred.Nap.18Dec.frag48.PAR.NEC.inv, Q2.5 = 1-(pred.Nap.18Dec.frag48.PAR.NEC.inv$Q2.5))
head(pred.Nap.18Dec.frag48.PAR.NEC.inv)
```

```{r}
pred.Nap.18Dec.frag48.UV.NEC.inv <- as.data.frame(pred.Nap.18Dec.frag48.UV)
head(pred.Nap.18Dec.frag48.UV.NEC.inv)
pred.Nap.18Dec.frag48.UV.NEC.inv <- mutate(pred.Nap.18Dec.frag48.UV.NEC.inv, Estimate = 1-(pred.Nap.18Dec.frag48.UV.NEC.inv$Estimate))
pred.Nap.18Dec.frag48.UV.NEC.inv <- mutate(pred.Nap.18Dec.frag48.UV.NEC.inv, Q97.5 = 1-(pred.Nap.18Dec.frag48.UV.NEC.inv$Q97.5))
pred.Nap.18Dec.frag48.UV.NEC.inv <- mutate(pred.Nap.18Dec.frag48.UV.NEC.inv, Q2.5 = 1-(pred.Nap.18Dec.frag48.UV.NEC.inv$Q2.5))
head(pred.Nap.18Dec.frag48.UV.NEC.inv)
```

Update predicted value RData file with inverted predicted values and CIs
```{r}
save(pred.Nap.18Dec.frag48, pred.Nap.18Dec.frag48.PAR, pred.Nap.18Dec.frag48.UV, pred.Nap.18Dec.frag48.PAR.NEC, pred.Nap.18Dec.frag48.UV.NEC, pred.Nap.18Dec.frag48.PAR.inv, pred.Nap.18Dec.frag48.UV.inv, pred.Nap.18Dec.frag48.PAR.NEC.inv, pred.Nap.18Dec.frag48.UV.NEC.inv, file = "2018.12_Nap_frag48_predvals.RData")
```
...
diff.prob.Nap.18Dec.frag48 <- max(pred.Nap.18Dec.frag48$prob_diff$prob)
diff.prob.Nap.18Dec.frag48
```
...
pred.Nap.18Dec.frag48$prob_diff$inv.x <- 10^pred.Nap.18Dec.frag48$prob_diff$x

p.comp.prob= ggplot(pred.Nap.18Dec.frag48$prob_diff) +
...
plot.Nap.18Dec.frag48.diff.prob <- p.comp.prob
plot.Nap.18Dec.frag48.diff.prob
```
...
comp.Nap.18Dec.frag48.EC50 <- compare_posterior(x=list("PAR" = out.Nap.18Dec.frag48.PAR, "UVR" = out.Nap.18Dec.frag48.UV), comparison = "ecx", ecx_val = 50)
save(comp.Nap.18Dec.frag48.EC50, file = "2018.12_Nap_frag48_posterior_comp_EC50.RData")
comp.Nap.18Dec.frag48.EC50$prob_diff
```
...
*Is EC50 +UVR lower than the EC50 -UVR?*
    - Unknown, there are no EC50s to compare.
...
comp.Nap.18Dec.frag48.NEC <- compare_posterior(x=list("PAR" = out.Nap.18Dec.frag48.PAR, "UVR" = out.Nap.18Dec.frag48.UV), comparison = "nec")
save(comp.Nap.18Dec.frag48.NEC, file = "2018.12_Nap_frag48_posterior_comp_NEC.RData")
comp.Nap.18Dec.frag48.NEC$prob_diff
```

    Table shows the % likelihood that the point estimate calculated (NEC, EC/EC10 or EC/EC50) for model A (in top row) is smaller than the point estimate calculated for model B, C, etc (in left hand column). 

*Is the NEC +UVR lower than the NEC -UVR?*
    - Yes/No, with a likelihood of 64.15%.
...
p.comp.NEC.diff = ggplot(comp.Nap.18Dec.frag48.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.Nap.18Dec.frag48.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.Nap.18Dec.frag48.NEC.diff <- p.comp.NEC.diff
comp.plot.Nap.18Dec.frag48.NEC.diff
```
...
```{r}
layout.Nap.18Dec.frag48 <- ggarrange(plot.Nap.18Dec.frag48.all, plot.Nap.18Dec.frag48.NEC, comp.plot.Nap.18Dec.frag48.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.Nap.18Dec.frag48
```
