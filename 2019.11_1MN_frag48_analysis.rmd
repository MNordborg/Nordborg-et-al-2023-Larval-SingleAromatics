---
title: "2017.11 Larval 1-methylnaphthalene 48 h fragmentation"
...
```{r}
data.1MN.19Nov.frag48.raw<- read_csv("Data/2019.11_AM_1-MN_frags48h.csv") %>% 
  data.frame() %>% 
...
save(data.1MN.19Nov.frag48.raw, file = "2019.11_1MN_data_frag48_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.1MN.19Nov.frag48.raw)
head(data.1MN.19Nov.frag48.raw)
tail(data.1MN.19Nov.frag48.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2019.11_1MN_data_frag48_raw.RData")

#Remove treatment replicates from data set
data.1MN.19Nov.frag48.controls <- data.1MN.19Nov.frag48.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.1MN.19Nov.frag48.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()


#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.1MN.19Nov.frag48.controls, family = binomial())
...
data.1MN.19Nov.frag48 <- data.1MN.19Nov.frag48.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.1MN.19Nov.frag48.PAR.raw <- data.1MN.19Nov.frag48 %>% 
  dplyr::filter(factor=="PAR") 
data.1MN.19Nov.frag48.PAR <- data.1MN.19Nov.frag48.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.1MN.19Nov.frag48.UV.raw <- data.1MN.19Nov.frag48 %>% 
  dplyr::filter(factor=="UVR") 
data.1MN.19Nov.frag48.UV <- data.1MN.19Nov.frag48.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.1MN.19Nov.frag48.PAR)
View(data.1MN.19Nov.frag48.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.1MN.19Nov.frag48.PAR.raw, data.1MN.19Nov.frag48.UV.raw, data.1MN.19Nov.frag48.PAR,data.1MN.19Nov.frag48.UV, file = "2019.11_1MN_data_frag48_filtered.RData")

load(file = "2019.11_1MN_data_frag48_filtered.RData")
```
...
unique(data.1MN.19Nov.frag48.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.1MN.19Nov.frag48.PAR$raw.x)
hist(data.1MN.19Nov.frag48.PAR$suc/data.1MN.19Nov.frag48.PAR$tot)
par(mfrow=c(1,1))
plot(data.1MN.19Nov.frag48.PAR$log.x, data.1MN.19Nov.frag48.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.frag48.PAR.all <- bnec(data = data.1MN.19Nov.frag48.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.1MN.19Nov.frag48.PAR.all, file = "2019.11_1MN_frag48_PAR_modfit_all.RData")
beep("mario")
```

Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_1MN_frag48_PAR_modfit_all.RData")

rhat(out.1MN.19Nov.frag48.PAR.all, rhat_cutoff = 1.03)
```
...
out.1MN.19Nov.frag48.PAR.all$mod_stats
summary(out.1MN.19Nov.frag48.PAR.all)
```

*Any evidence of overdispersion?*
    - All candidate models were overdispersed.
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.frag48.PAR.all.od <- bnec(data = data.1MN.19Nov.frag48.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.1MN.19Nov.frag48.PAR.all.od, file = "2019.11_1MN_frag48_PAR_modfit_all_OD.RData")
beep("mario")
```

No candidate models could be successfully fitted using the binomial distribution, therefore the MANEC fitted using the binomial distribution was used (despite overdispersion).

###### Model selection (binomial vs betabinomial)

####### Continue assessing the binomial MANEC fit

Check chain mixing
```{r}
load("2019.11_1MN_frag48_PAR_modfit_all.RData")

check_chains(out.1MN.19Nov.frag48.PAR.all, filename = "2019.11_1MN_frag48_PAR_chains_modfit_all") #change to out.1MN.19Nov.frag48.PAR.all2 if any models were dropped based on rhat
```
...
```{r}
plot(out.1MN.19Nov.frag48.PAR.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
pdf("2019.11_1MN_frag48_PAR_modfitplots_all.pdf")
plot(out.1MN.19Nov.frag48.PAR.all, all_models = TRUE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```

Rename model output for selected model (binomial or betabinomial)
```{r}
out.1MN.19Nov.frag48.PAR <- out.1MN.19Nov.frag48.PAR.all
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.1MN.19Nov.frag48.PAR.NEC <- pull_out(out.1MN.19Nov.frag48.PAR, model = c("nec"))
```


```{r}
plot(out.1MN.19Nov.frag48.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.1MN.19Nov.frag48.PAR, out.1MN.19Nov.frag48.PAR.NEC, file = "2019.11_1MN_frag48_PAR_modfits_final.RData")
load("2019.11_1MN_frag48_PAR_modfits_final.RData")
```
...
```{r}
load("2019.11_1MN_frag48_PAR_modfits_final.RData")
NEC.1MN.19Nov.frag48.PAR <- out.1MN.19Nov.frag48.PAR.NEC$w_nec
NEC.1MN.19Nov.frag48.PAR <- 10^NEC.1MN.19Nov.frag48.PAR #if using log10(raw.x)
#NEC.1MN.19Nov.frag48.PAR <- exp(NEC.1MN.19Nov.frag48.PAR) #if using log(raw.x)
NEC.1MN.19Nov.frag48.PAR
```

```{r}
load("2019.11_1MN_frag48_PAR_modfits_final.RData")
EC10.1MN.19Nov.frag48.PAR <- ecx(out.1MN.19Nov.frag48.PAR, ecx_val = 10, type = "absolute")
EC10.1MN.19Nov.frag48.PAR <- 10^EC10.1MN.19Nov.frag48.PAR #if using log10(raw.x)
#EC10.1MN.19Nov.frag48.PAR <- exp(EC10.1MN.19Nov.frag48.PAR) #if using log(raw.x)
EC10.1MN.19Nov.frag48.PAR
```

```{r}
load("2019.11_1MN_frag48_PAR_modfits_final.RData")
EC50.1MN.19Nov.frag48.PAR <- ecx(out.1MN.19Nov.frag48.PAR, ecx_val = 50, type = "absolute")
EC50.1MN.19Nov.frag48.PAR <- 10^EC50.1MN.19Nov.frag48.PAR #if using log10(raw.x)
#EC50.1MN.19Nov.frag48.PAR <- exp(EC50.1MN.19Nov.frag48.PAR) #if using log(raw.x)
EC50.1MN.19Nov.frag48.PAR
```
...
unique(data.1MN.19Nov.frag48.UV$raw.x)
par(mfrow=c(2,1))
hist(data.1MN.19Nov.frag48.UV$raw.x)
hist(data.1MN.19Nov.frag48.UV$suc/data.1MN.19Nov.frag48.UV$tot)
par(mfrow=c(1,1))
plot(data.1MN.19Nov.frag48.UV$log.x, data.1MN.19Nov.frag48.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.frag48.UV.all <- bnec(data = data.1MN.19Nov.frag48.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.1MN.19Nov.frag48.UV.all, file = "2019.11_1MN_frag48_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_1MN_frag48_UV_modfit_all.RData")

rhat(out.1MN.19Nov.frag48.UV.all, rhat_cutoff = 1.03)
```
...
out.1MN.19Nov.frag48.UV.all$mod_stats
summary(out.1MN.19Nov.frag48.UV.all)
```
...
```{r}
plot(out.1MN.19Nov.frag48.UV.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.frag48.UV.all.od <- bnec(data = data.1MN.19Nov.frag48.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.1MN.19Nov.frag48.UV.all.od, file = "2019.11_1MN_frag48_UV_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_1MN_frag48_UV_modfit_all_OD.RData")

rhat(out.1MN.19Nov.frag48.UV.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.1MN.19Nov.frag48.UV.all.od, filename = "2019.11_1MN_frag48_UV_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: out.1MN.19Nov.frag48.UV.all.od2
out.1MN.19Nov.frag48.UV.all.od$mod_stats
summary(out.1MN.19Nov.frag48.UV.all.od)
```



```{r}
plot(out.1MN.19Nov.frag48.UV.all.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2019.11_1MN_frag48_UV_modfit_all_OD.RData")
pdf("2019.11_1MN_frag48_UV_modfitplots_all_OD.pdf")
plot(out.1MN.19Nov.frag48.UV.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
plot(out.1MN.19Nov.frag48.UV.all, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10)
plot(out.1MN.19Nov.frag48.UV.all.od, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10)
```
...
*Did using the betabinomial distribution improve MANEC model fit compared to the MANEC fit using the binomial distribution?*
    - Unclear as data is very sparse. However, the overdispersion for the binomial MANEC indicates this is an unsuitable distribution.
    

Rename model output for selected model (binomial or betabinomial)
```{r}
out.1MN.19Nov.frag48.UV <- out.1MN.19Nov.frag48.UV.all.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.1MN.19Nov.frag48.UV.NEC <- pull_out(out.1MN.19Nov.frag48.UV, model = c("nec"))
```


```{r}
plot(out.1MN.19Nov.frag48.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.1MN.19Nov.frag48.UV, out.1MN.19Nov.frag48.UV.NEC, file = "2019.11_1MN_frag48_UV_modfits_final.RData")
load("2019.11_1MN_frag48_UV_modfits_final.RData")
```
...
```{r}
load("2019.11_1MN_frag48_UV_modfits_final.RData")
NEC.1MN.19Nov.frag48.UV <- out.1MN.19Nov.frag48.UV.NEC$w_nec
NEC.1MN.19Nov.frag48.UV <- 10^NEC.1MN.19Nov.frag48.UV #if using log10(raw.x)
#NEC.1MN.19Nov.frag48.UV <- exp(NEC.1MN.19Nov.frag48.UV) #if using log(raw.x)
NEC.1MN.19Nov.frag48.UV
```

```{r}
load("2019.11_1MN_frag48_UV_modfits_final.RData")
EC10.1MN.19Nov.frag48.UV <- ecx(out.1MN.19Nov.frag48.UV, ecx_val = 10, type = "absolute")
EC10.1MN.19Nov.frag48.UV <- 10^EC10.1MN.19Nov.frag48.UV #if using log10(raw.x)
#EC10.1MN.19Nov.frag48.UV <- exp(EC10.1MN.19Nov.frag48.UV) #if using log(raw.x)
EC10.1MN.19Nov.frag48.UV
```

```{r}
load("2019.11_1MN_frag48_UV_modfits_final.RData")
EC50.1MN.19Nov.frag48.UV <- ecx(out.1MN.19Nov.frag48.UV, ecx_val = 50, type = "absolute")
EC50.1MN.19Nov.frag48.UV <- 10^EC50.1MN.19Nov.frag48.UV #if using log10(raw.x)
#EC50.1MN.19Nov.frag48.UV <- exp(EC50.1MN.19Nov.frag48.UV) #if using log(raw.x)
EC50.1MN.19Nov.frag48.UV
```
...
pred.1MN.19Nov.frag48 <- compare_posterior(x=list("PAR" = out.1MN.19Nov.frag48.PAR, "UVR" = out.1MN.19Nov.frag48.UV), comparison = "fitted", precision = 50)

pred.1MN.19Nov.frag48.PAR <- pred.1MN.19Nov.frag48$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.1MN.19Nov.frag48.UV <- pred.1MN.19Nov.frag48$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.1MN.19Nov.frag48.NEC <- compare_posterior(list("PAR" = out.1MN.19Nov.frag48.PAR.NEC, "UVR" = out.1MN.19Nov.frag48.UV.NEC), comparison = "fitted", precision = 50)

pred.1MN.19Nov.frag48.PAR.NEC <- pred.1MN.19Nov.frag48.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.1MN.19Nov.frag48.UV.NEC <- pred.1MN.19Nov.frag48.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.1MN.19Nov.frag48, pred.1MN.19Nov.frag48.PAR, pred.1MN.19Nov.frag48.UV, pred.1MN.19Nov.frag48.PAR.NEC, pred.1MN.19Nov.frag48.UV.NEC, file = "2019.11_1MN_frag48_predvals.RData") 

load("2019.11_1MN_frag48_predvals.RData")
str(pred.1MN.19Nov.frag48.PAR)
str(pred.1MN.19Nov.frag48.UV)
```
...
Full subset MANECs
```{r}
pred.1MN.19Nov.frag48.PAR.inv <- as.data.frame(pred.1MN.19Nov.frag48.PAR)
head(pred.1MN.19Nov.frag48.PAR.inv)
pred.1MN.19Nov.frag48.PAR.inv <- mutate(pred.1MN.19Nov.frag48.PAR.inv, Estimate = 1-(pred.1MN.19Nov.frag48.PAR.inv$Estimate))
pred.1MN.19Nov.frag48.PAR.inv <- mutate(pred.1MN.19Nov.frag48.PAR.inv, Q97.5 = 1-(pred.1MN.19Nov.frag48.PAR.inv$Q97.5))
pred.1MN.19Nov.frag48.PAR.inv <- mutate(pred.1MN.19Nov.frag48.PAR.inv, Q2.5 = 1-(pred.1MN.19Nov.frag48.PAR.inv$Q2.5))
head(pred.1MN.19Nov.frag48.PAR.inv)
```

```{r}
pred.1MN.19Nov.frag48.UV.inv <- as.data.frame(pred.1MN.19Nov.frag48.UV)
head(pred.1MN.19Nov.frag48.UV.inv)
pred.1MN.19Nov.frag48.UV.inv <- mutate(pred.1MN.19Nov.frag48.UV.inv, Estimate = 1-(pred.1MN.19Nov.frag48.UV.inv$Estimate))
pred.1MN.19Nov.frag48.UV.inv <- mutate(pred.1MN.19Nov.frag48.UV.inv, Q97.5 = 1-(pred.1MN.19Nov.frag48.UV.inv$Q97.5))
pred.1MN.19Nov.frag48.UV.inv <- mutate(pred.1MN.19Nov.frag48.UV.inv, Q2.5 = 1-(pred.1MN.19Nov.frag48.UV.inv$Q2.5))
head(pred.1MN.19Nov.frag48.UV.inv)
```

NEC subset MANECs
```{r}
pred.1MN.19Nov.frag48.PAR.NEC.inv <- as.data.frame(pred.1MN.19Nov.frag48.PAR)
head(pred.1MN.19Nov.frag48.PAR.NEC.inv)
pred.1MN.19Nov.frag48.PAR.NEC.inv <- mutate(pred.1MN.19Nov.frag48.PAR.NEC.inv, Estimate = 1-(pred.1MN.19Nov.frag48.PAR.NEC.inv$Estimate))
pred.1MN.19Nov.frag48.PAR.NEC.inv <- mutate(pred.1MN.19Nov.frag48.PAR.NEC.inv, Q97.5 = 1-(pred.1MN.19Nov.frag48.PAR.NEC.inv$Q97.5))
pred.1MN.19Nov.frag48.PAR.NEC.inv <- mutate(pred.1MN.19Nov.frag48.PAR.NEC.inv, Q2.5 = 1-(pred.1MN.19Nov.frag48.PAR.NEC.inv$Q2.5))
head(pred.1MN.19Nov.frag48.PAR.NEC.inv)
```

```{r}
pred.1MN.19Nov.frag48.UV.NEC.inv <- as.data.frame(pred.1MN.19Nov.frag48.UV)
head(pred.1MN.19Nov.frag48.UV.NEC.inv)
pred.1MN.19Nov.frag48.UV.NEC.inv <- mutate(pred.1MN.19Nov.frag48.UV.NEC.inv, Estimate = 1-(pred.1MN.19Nov.frag48.UV.NEC.inv$Estimate))
pred.1MN.19Nov.frag48.UV.NEC.inv <- mutate(pred.1MN.19Nov.frag48.UV.NEC.inv, Q97.5 = 1-(pred.1MN.19Nov.frag48.UV.NEC.inv$Q97.5))
pred.1MN.19Nov.frag48.UV.NEC.inv <- mutate(pred.1MN.19Nov.frag48.UV.NEC.inv, Q2.5 = 1-(pred.1MN.19Nov.frag48.UV.NEC.inv$Q2.5))
head(pred.1MN.19Nov.frag48.UV.NEC.inv)
```


Update RData file for predicted value objects with inverted predictions
```{r}
save(pred.1MN.19Nov.frag48, pred.1MN.19Nov.frag48.PAR, pred.1MN.19Nov.frag48.UV, pred.1MN.19Nov.frag48.PAR.NEC, pred.1MN.19Nov.frag48.UV.NEC, pred.1MN.19Nov.frag48.PAR.inv, pred.1MN.19Nov.frag48.UV.inv, pred.1MN.19Nov.frag48.PAR.NEC.inv, pred.1MN.19Nov.frag48.UV.NEC.inv, file = "2019.11_1MN_frag48_predvals.RData") 
```
...
diff.prob.1MN.19Nov.frag48 <- max(pred.1MN.19Nov.frag48$prob_diff$prob)
diff.prob.1MN.19Nov.frag48
```
...
pred.1MN.19Nov.frag48$prob_diff$inv.x <- 10^pred.1MN.19Nov.frag48$prob_diff$x

p.comp.prob= ggplot(pred.1MN.19Nov.frag48$prob_diff) +
...
plot.1MN.19Nov.frag48.diff.prob <- p.comp.prob
plot.1MN.19Nov.frag48.diff.prob
```
...
No EC50s could be derived therefore no comparison was performed.
...
comp.1MN.19Nov.frag48.NEC <- compare_posterior(x=list("PAR" = out.1MN.19Nov.frag48.PAR, "UVR" = out.1MN.19Nov.frag48.UV), comparison = "nec")
save(comp.1MN.19Nov.frag48.NEC, file = "2019.11_1MN_frag48_posterior_comp_NEC.RData")
comp.1MN.19Nov.frag48.NEC$prob_diff
```
...
*Is the NEC +UVR lower than the NEC -UVR?*
    - No, the likelihood that UV NEC < PAR NEC is 51.03%.
...
p.comp.NEC.diff = ggplot(comp.1MN.19Nov.frag48.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.1MN.19Nov.frag48.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.1MN.19Nov.frag48.NEC.diff <- p.comp.NEC.diff
comp.plot.1MN.19Nov.frag48.NEC.diff
```
...
```{r}
layout.1MN.19Nov.frag48 <- ggarrange(plot.1MN.19Nov.frag48.all, plot.1MN.19Nov.frag48.NEC, comp.plot.1MN.19Nov.frag48.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.1MN.19Nov.frag48
```
