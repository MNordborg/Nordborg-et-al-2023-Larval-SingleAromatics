---
title: "2019.11 Larval 1-methylnaphthalene survival"
...
```{r}
data.1MN.19Nov.surv.raw<- read_csv("Data/2019.11_AM_1-MN_surv.csv") %>% 
  data.frame() %>% 
...
save(data.1MN.19Nov.surv.raw, file = "2019.11_1MN_data_surv_raw.RData")

#Check that all columns have been assigned the correct data type & that adding 0.1 to raw.x controls worked
str(data.1MN.19Nov.surv.raw)
head(data.1MN.19Nov.surv.raw)
tail(data.1MN.19Nov.surv.raw)
```


Check for differences between 0.01% DMSO and FSW controls
```{r}
load("2019.11_1MN_data_surv_raw.RData")

#Remove treatment replicates from data set
data.1MN.19Nov.surv.controls <- data.1MN.19Nov.surv.raw %>% 
  dplyr::filter(nom.x!="Treatment")

#Plot to compare means and CIs
ggplot(data.1MN.19Nov.surv.controls, aes(x=nom.x, y=prop, fill=factor)) + 
    geom_boxplot()

#Filer in preparation for t-tests
#MN.19Nov.surv.controls.PAR.FSW <- data.1MN.19Nov.surv.controls %>% 
  #dplyr::filter(factor=="PAR" & nom.x=="FSW")
#MN.19Nov.surv.controls.PAR.DMSO <- data.1MN.19Nov.surv.controls %>% 
  #dplyr::filter(factor=="PAR" & nom.x!="FSW")

#MN.19Nov.surv.controls.UV.FSW <- data.1MN.19Nov.surv.controls %>% 
 # dplyr::filter(factor=="UVR" & nom.x=="FSW")
#MN.19Nov.surv.controls.UV.DMSO <- data.1MN.19Nov.surv.controls %>% 
 # dplyr::filter(factor=="UVR" & nom.x!="FSW")

#Perform t-test to check for differences between control groups
brms.fit <- brm(suc | trials(tot)  ~ nom.x*factor, data = data.1MN.19Nov.surv.controls, family = binomial())
...
data.1MN.19Nov.surv <- data.1MN.19Nov.surv.raw %>% 
  dplyr::filter(nom.x!="FSW")

#Create -UVR data set and prep for use in model fitting
data.1MN.19Nov.surv.PAR.raw <- data.1MN.19Nov.surv %>% 
  dplyr::filter(factor=="PAR") 
data.1MN.19Nov.surv.PAR <- data.1MN.19Nov.surv.PAR.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#Create +UVR data set and prep for use in model fitting
data.1MN.19Nov.surv.UV.raw <- data.1MN.19Nov.surv %>% 
  dplyr::filter(factor=="UVR") 
data.1MN.19Nov.surv.UV <- data.1MN.19Nov.surv.UV.raw %>% 
  dplyr::filter(use_in_model=="Yes") 

#check that filtering worked
View(data.1MN.19Nov.surv.PAR)
View(data.1MN.19Nov.surv.UV)

#Save complete data sets and data sets prepared for model fitting
save(data.1MN.19Nov.surv.PAR.raw, data.1MN.19Nov.surv.UV.raw, data.1MN.19Nov.surv.PAR,data.1MN.19Nov.surv.UV, file = "2019.11_1MN_data_surv_filtered.RData")

load(file = "2019.11_1MN_data_surv_filtered.RData")
```
...
unique(data.1MN.19Nov.surv.PAR$raw.x)
par(mfrow=c(2,1))
hist(data.1MN.19Nov.surv.PAR$raw.x)
hist(data.1MN.19Nov.surv.PAR$suc/data.1MN.19Nov.surv.PAR$tot)
par(mfrow=c(1,1))
plot(data.1MN.19Nov.surv.PAR$log.x, data.1MN.19Nov.surv.PAR$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.surv.PAR.all <- bnec(data = data.1MN.19Nov.surv.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")
save(out.1MN.19Nov.surv.PAR.all, file = "2019.11_1MN_surv_PAR_modfit_all.RData")
beep("mario")
```
...
```{r}
load("2019.11_1MN_surv_PAR_modfit_all.RData")

rhat(out.1MN.19Nov.surv.PAR.all, rhat_cutoff = 1.03)
```
...
out.1MN.19Nov.surv.PAR.all$mod_stats
summary(out.1MN.19Nov.surv.PAR.all)
```

*Any evidence of overdispersion?*
    - All candidate models show indications of overdispersion.



```{r}
plot(out.1MN.19Nov.surv.PAR.all, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```    
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.surv.PAR.all.od <- bnec(data = data.1MN.19Nov.surv.PAR, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.1MN.19Nov.surv.PAR.all.od, file = "2019.11_1MN_surv_PAR_modfit_all_OD.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_1MN_surv_PAR_modfit_all_OD.RData")
rhat(out.1MN.19Nov.surv.PAR.all.od, rhat_cutoff = 1.03)
```
...
check_chains(out.1MN.19Nov.surv.PAR.all.od, filename = "2019.11_1MN_surv_PAR_chains_modfit_all_OD") #change this if any candidate models were discarded based on Rhat: 
out.1MN.19Nov.surv.PAR.all.od$mod_stats
summary(out.1MN.19Nov.surv.PAR.all.od)
```
...
```{r}
plot(out.1MN.19Nov.surv.PAR.all.od, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
```{r}
load("2019.11_1MN_surv_PAR_modfit_all_OD.RData")
pdf("2019.11_1MN_surv_PAR_modfitplots_all_OD.pdf")
plot(out.1MN.19Nov.surv.PAR.all.od, all_models = TRUE, add_ec10 = FALSE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
*Did using the betabinomial distribution improve MANEC model fit compared to the MANEC fit using the binomial distribution?*
    -> Credible intervals became more reasonable but the median line appears more suitable for the binomial. 


Rename model output for selected model (binomial or betabinomial)
```{r}
out.1MN.19Nov.surv.PAR <- out.1MN.19Nov.surv.PAR.all.od
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.1MN.19Nov.surv.PAR.NEC <- pull_out(out.1MN.19Nov.surv.PAR, model = c("nec"))
```


```{r}
plot(out.1MN.19Nov.surv.PAR.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.1MN.19Nov.surv.PAR, out.1MN.19Nov.surv.PAR.NEC, file = "2019.11_1MN_surv_PAR_modfits_final.RData")
load("2019.11_1MN_surv_PAR_modfits_final.RData")
```
...
```{r}
load("2019.11_1MN_surv_PAR_modfits_final.RData")
NEC.1MN.19Nov.surv.PAR <- out.1MN.19Nov.surv.PAR.NEC$w_nec
NEC.1MN.19Nov.surv.PAR <- 10^NEC.1MN.19Nov.surv.PAR #if using log10(raw.x)
#NEC.1MN.19Nov.surv.PAR <- exp(NEC.1MN.19Nov.surv.PAR) #if using log(raw.x))
NEC.1MN.19Nov.surv.PAR
```

```{r}
load("2019.11_1MN_surv_PAR_modfits_final.RData")
LC10.1MN.19Nov.surv.PAR <- ecx(out.1MN.19Nov.surv.PAR, ecx_val = 10, type = "absolute")
LC10.1MN.19Nov.surv.PAR <- 10^LC10.1MN.19Nov.surv.PAR #if using log10(raw.x)
#LC10.1MN.19Nov.surv.PAR <- exp(LC10.1MN.19Nov.surv.PAR) #if using log(raw.x))
LC10.1MN.19Nov.surv.PAR
```

```{r}
load("2019.11_1MN_surv_PAR_modfits_final.RData")
LC50.1MN.19Nov.surv.PAR <- ecx(out.1MN.19Nov.surv.PAR, ecx_val = 50, type = "absolute")
LC50.1MN.19Nov.surv.PAR <- 10^LC50.1MN.19Nov.surv.PAR #if using log10(raw.x)
#LC50.1MN.19Nov.surv.PAR <- exp(LC50.1MN.19Nov.surv.PAR) #if using log(raw.x))
LC50.1MN.19Nov.surv.PAR
```
...
unique(data.1MN.19Nov.surv.UV$raw.x)
par(mfrow=c(2,1))
hist(data.1MN.19Nov.surv.UV$raw.x)
hist(data.1MN.19Nov.surv.UV$suc/data.1MN.19Nov.surv.UV$tot)
par(mfrow=c(1,1))
plot(data.1MN.19Nov.surv.UV$log.x, data.1MN.19Nov.surv.UV$prop) #use this to get an initial visual idea of priors to use (if setting priors manually)
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.surv.UV.all <- bnec(data = data.1MN.19Nov.surv.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all")

save(out.1MN.19Nov.surv.UV.all, file = "2019.11_1MN_surv_UV_modfit_all.RData")
beep("mario")
```
...
Check RHat values to discard bad candidate model fits
```{r}
load("2019.11_1MN_surv_UV_modfit_all.RData")

rhat(out.1MN.19Nov.surv.UV.all, rhat_cutoff = 1.03)
```
...
out.1MN.19Nov.surv.UV.all2 <- amend(out.1MN.19Nov.surv.UV.all, drop = c("nec4param")) #add in bad candidate models in drop = c())
save(out.1MN.19Nov.surv.UV.all, out.1MN.19Nov.surv.UV.all2, file = "2019.11_1MN_surv_UV_modfit_all.RData")
```
...
out.1MN.19Nov.surv.UV.all2$mod_stats
summary(out.1MN.19Nov.surv.UV.all2)
```

*Any evidence of overdispersion?*
    - All candidate models show some evidence of overdispersion
    
    However, no MANEC could be successfully fitted using the betabinomial distribution. Therefore the MANEC fitted usng the binomial distribution will be used to extract results (if applicable) and produce graphical representations.
...
check_chains(out.1MN.19Nov.surv.UV.all2, filename = "2019.11_1MN_surv_UV_chains_modfit_all") #change to out.1MN.19Nov.surv.PAR.all2 if any models were dropped based on rhat
```
...
```{r}
plot(out.1MN.19Nov.surv.UV.all2, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
pdf("2019.11_1MN_surv_UV_modfitplots_all.pdf")
plot(out.1MN.19Nov.surv.UV.all2, all_models = TRUE, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
dev.off()
```
...
```{r}
out.1MN.19Nov.surv.UV.mod <- amend(out.1MN.19Nov.surv.UV.all2, drop = c("nechorme4", "ecxll5"))
```

Check the modified MANEC fit
```{r}
plot(out.1MN.19Nov.surv.UV.mod, add_ec10 = TRUE, add_nec = FALSE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```
...
library(bayesnec)
library(beepr)
out.1MN.19Nov.surv.UV.all.od <- bnec(data = data.1MN.19Nov.surv.UV, 
                        x_var = "log.x", 
                        y_var = "suc", 
                        trials_var = "tot",
                        model = "all",
...
save(out.1MN.19Nov.surv.UV.all.od, file = "2019.11_1MN_surv_UV_modfit_all_OD.RData")
beep("mario")
```


   MANEC using the betabinomial distribution could not be successfully fitted.
      ->The MANEC fitted using the binomial distribution used.
...
Rename model output for selected model (binomial or betabinomial)
```{r}
out.1MN.19Nov.surv.UV <- out.1MN.19Nov.surv.UV.mod
```


Extract the NEC-model subset for use in extraction of NEC and comparisons of posterior distributions
```{r}
out.1MN.19Nov.surv.UV.NEC <- pull_out(out.1MN.19Nov.surv.UV, model = c("nec"))
```


```{r}
plot(out.1MN.19Nov.surv.UV.NEC, add_nec = TRUE, lxform = inv.log10) #if modelled on log(raw.x) use: lxform = exp
```


Save the output for the selected models
```{r}
save(out.1MN.19Nov.surv.UV, out.1MN.19Nov.surv.UV.NEC, file = "2019.11_1MN_surv_UV_modfits_final.RData")
load("2019.11_1MN_surv_UV_modfits_final.RData")
```
...
```{r}
load("2019.11_1MN_surv_UV_modfits_final.RData")
NEC.1MN.19Nov.surv.UV <- out.1MN.19Nov.surv.UV.NEC$w_nec
NEC.1MN.19Nov.surv.UV <- 10^NEC.1MN.19Nov.surv.UV #if using log10(raw.x)
#NEC.1MN.19Nov.surv.UV <- exp(NEC.1MN.19Nov.surv.UV) #if using log(raw.x)
NEC.1MN.19Nov.surv.UV
```

```{r}
load("2019.11_1MN_surv_UV_modfits_final.RData")
LC10.1MN.19Nov.surv.UV <- ecx(out.1MN.19Nov.surv.UV, ecx_val = 10, type = "absolute")
LC10.1MN.19Nov.surv.UV <- 10^LC10.1MN.19Nov.surv.UV #if using log10(raw.x)
#LC10.1MN.19Nov.surv.UV <- exp(LC10.1MN.19Nov.surv.UV) #if using log(raw.x)
LC10.1MN.19Nov.surv.UV
```

```{r}
load("2019.11_1MN_surv_UV_modfits_final.RData")
LC50.1MN.19Nov.surv.UV <- ecx(out.1MN.19Nov.surv.UV, ecx_val = 50, type = "absolute")
LC50.1MN.19Nov.surv.UV <- 10^LC50.1MN.19Nov.surv.UV #if using log10(raw.x)
#LC50.1MN.19Nov.surv.UV <- exp(LC50.1MN.19Nov.surv.UV) #if using log(raw.x)
LC50.1MN.19Nov.surv.UV
```
...
pred.1MN.19Nov.surv <- compare_posterior(x=list("PAR" = out.1MN.19Nov.surv.PAR, "UVR" = out.1MN.19Nov.surv.UV), comparison = "fitted", precision = 50)

pred.1MN.19Nov.surv.PAR <- pred.1MN.19Nov.surv$posterior_data %>% 
  dplyr::filter(model=="PAR")

pred.1MN.19Nov.surv.UV <- pred.1MN.19Nov.surv$posterior_data %>% 
  dplyr::filter(model=="UVR")



pred.1MN.19Nov.surv.NEC <- compare_posterior(list("PAR" = out.1MN.19Nov.surv.PAR.NEC, "UVR" = out.1MN.19Nov.surv.UV.NEC), comparison = "fitted", precision = 50)

pred.1MN.19Nov.surv.PAR.NEC <- pred.1MN.19Nov.surv.NEC$posterior_data %>% 
  dplyr::filter(model=="PAR")
pred.1MN.19Nov.surv.UV.NEC <- pred.1MN.19Nov.surv.NEC$posterior_data %>% 
 dplyr::filter(model=="UVR")


save(pred.1MN.19Nov.surv, pred.1MN.19Nov.surv.PAR, pred.1MN.19Nov.surv.UV, pred.1MN.19Nov.surv.PAR.NEC, pred.1MN.19Nov.surv.UV.NEC, file = "2019.11_1MN_surv_predvals.RData") # pred.1MN.19Nov.surv.NEC, pred.1MN.19Nov.surv.PAR.NEC, pred.1MN.19Nov.surv.UV.NEC <- add these back in once UV-section is working

load("2019.11_1MN_surv_predvals.RData")
str(pred.1MN.19Nov.surv.PAR)
str(pred.1MN.19Nov.surv.UV)
```
...
p.all = p.all+ labs(x=expression(), #Concentration~(μg~"L"^{-1})
              ...
              subtitle="a) 1-methylnaphthalene")
...
diff.prob.1MN.19Nov.surv <- max(pred.1MN.19Nov.surv$prob_diff$prob)
diff.prob.1MN.19Nov.surv
```
...
pred.1MN.19Nov.surv$prob_diff$inv.x <- 10^pred.1MN.19Nov.surv$prob_diff$x

p.comp.prob= ggplot(pred.1MN.19Nov.surv$prob_diff) +
...
plot.1MN.19Nov.surv.diff.prob <- p.comp.prob
plot.1MN.19Nov.surv.diff.prob
```
...
No comparison performed as no LC50 values could be derived.
...
comp.1MN.19Nov.surv.NEC <- compare_posterior(x=list("PAR" = out.1MN.19Nov.surv.PAR, "UVR" = out.1MN.19Nov.surv.UV), comparison = "nec")
save(comp.1MN.19Nov.surv.NEC, file = "2019.11_1MN_surv_posterior_comp_NEC.RData")
comp.1MN.19Nov.surv.NEC$prob_diff
```
...
*Is the NEC +UVR lower than the NEC -UVR?*
    - No, only a likelihood of 24.70% that +UV treatment is more toxic than -UV treatment.
...
p.comp.NEC.diff = ggplot(comp.1MN.19Nov.surv.NEC$diff_data, aes(x=diff)) + 
  geom_density(aes(x=diff, fill = 'grey'), alpha=0.4) + 
  stat_pointinterval(aes(y = 0.00, x = comp.1MN.19Nov.surv.NEC$df4.s$diff),.width = c(.66, .95)) +
...
#Rename & show plot
comp.plot.1MN.19Nov.surv.NEC.diff <- p.comp.NEC.diff
comp.plot.1MN.19Nov.surv.NEC.diff
```
...
```{r}
layout.1MN.19Nov.surv <- ggarrange(plot.1MN.19Nov.surv.all, plot.1MN.19Nov.surv.diff.est, plot.1MN.19Nov.surv.NEC, comp.plot.1MN.19Nov.surv.NEC,
          ncol = 2, nrow = 2,
          widths = c(0.5, 0.5))
layout.1MN.19Nov.surv
```
